...
#'   x<-saemixData(name.data=cow.saemix,name.group="cow",name.predictors=c("time"), name.response="weight",
#'   name.covariates=c("birthyear","twin","birthrank"), units=list(x="d",y="kg",covariates=c("yr","-","-")),verbose=F)
#'   x2<-transformContCov(x,birthyear,centering="median",transformation=log,verbose=F, newCovName = "logYear")
...
#' head(cowt@data) 
#' # the new covariates are birthrank.ref (initially 3), birthrank.G2 (regrouping 4 and 5) and birthrank.G3 (6 and 7)
#' # only birthrank.G2 and birthrank.G3 are included in the name.covariates slot of the object cowt
#' cowt <- transformCatCov(x, covariate=birthrank, newCat=c(1,2,2,3,3), newCatName=c("ref","preg4-5","6-7"), verbose=TRUE)
#' head(cowt@data) # new names can be assigned to the dichotomised covariates through the newCatName argument
#' # the new covariates are now "ref", preg4-5" (regrouping 4 and 5) and "6-7" (6 and 7)
#' 
#' # Changing the reference for a binary variable
#' cowt<-transformCatCov(x, covariate=twin, newCat=c(2,1), newCatName="Singleton", verbose=TRUE)
#' 
#' @keywords data
#' @export 
#' 
