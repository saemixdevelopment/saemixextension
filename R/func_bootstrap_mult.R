...
saemix.bootstrap<-function(saemixObject, method="conditional", nboot=200, nsamp=100, saemix.options=NULL) {
  if(method!="case") {
    if(sum(saemixObject@model@modeltype!="structural")>0 & (is.null(body(saemixObject@model@simulate.function)) | length(formals(saemixObject@model@simulate.function))!=3)) {
      if(saemixObject@options$warnings) message("A simulation function needs to be provided for models involving non Gaussian responses to obtain bootstrap distributions by other methods than the Case bootstrap. This function needs to have the same structure as the model function and return simulated values based on the same model. \nPlease provide a simulation function the simulate.function slot of the model or use method='case' for Case bootstrap. \nExiting bootstrap.")
      return(NULL)
    }
  }
  ...
  if(sum(saemixObject@model@modeltype=="structural")>0) idx.eps<-saemixObject@model@indx.res else idx.eps<-integer(0)
  ...
  for(iboot in 1:nboot) {
    ...
    if(is(fit.boot,"try-error")) { # try changing the initial parameters (for categorical models)
...
dataGen.NP<-function(saemixObject,nsamp=0,eta.sampc=NULL,conditional=FALSE) {
  ...
  ismodel.struct <- which(saemixObject@model@modeltype=="structural")
  id<-saemixObject@data@data$index
  xdep<-saemixObject@data@data[,c(saemixObject@data["name.predictors"],"ytype"),drop=FALSE]
  smx.data<-saemixObject@data
  if(sum(saemixObject@model@modeltype!="structural")>0) {
    fpred <- saemixObject@model@simulate.function(x$psi.boot,id,xdep)
  } else {
    fpred<-saemixObject@model@model(x$psi.boot,id,xdep)
  }
  if(length(ismodel.struct)>0) {
    idx.etyp<-which(saemixObject@model@error.model=="exponential")
    if(length(idx.etyp)>0)
      fpred[xdep$ytype %in%idx.etyp]<-log(cutoff(fpred[xdep$ytype %in%idx.etyp]))
    gpred<-error(fpred,saemixObject@results@respar,xdep$ytype)
    fpred[xdep$ytyp %in% ismodel.struct]<-fpred[xdep$ytyp %in% ismodel.struct]+gpred[xdep$ytyp %in% ismodel.struct]*x$eps.boot
  }
  smx.data@data[,saemixObject@data["name.response"]] <- fpred
  return(smx.data)
}
...
sampDist.Par <- function(saemixObject) {
  ...
  ismodel.struct <- which(saemixObject@model@modeltype=="structural")
  # eps.boot: NULL if all models are defined by their log-likelihood
  #         : 0 for all models defined by their log-likelihood and sampled from N(0,1) for Gaussian models 
  if(length(ismodel.struct)>0) {
    eps.boot<-rep(0,length(saemixObject@results@iwres))
    neps <- length(saemixObject@results@iwres[saemixObject@data@data$ytype %in% ismodel.struct] )
    eps.boot[saemixObject@data@data$ytype %in% ismodel.struct] <- rnorm(neps)
  } else eps.boot<-NULL
...
sampDist.NP <- function(saemixObject) {
  ...
  ismodel.struct <- which(saemixObject@model@modeltype=="structural")
  # eps.boot: NULL if all models are defined by their log-likelihood
  #         : 0 for all models defined by their log-likelihood and sampled from the corresponding (normalised) iwres for Gaussian models 
  if(length(ismodel.struct)>0) {
    eps.boot<-rep(0,length(saemixObject@results@iwres))
    for(ityp in ismodel.struct) {
      eps.est<-saemixObject@results@iwres[saemixObject@data@data$ytype==ityp] # Estimated epsilon_ij
      epsc<-center.eps(eps.est)
      epscorr<-epsc/sd(epsc)
      eps.boot[saemixObject@data@data$ytype==ityp] <- sample(epscorr,size=length(eps.est),replace=T)
    }
  } else eps.boot<-NULL
  phicond<-saemixObject@results@cond.mean.phi
  etacond<-saemixObject@results@cond.mean.eta # Estimated eta_i
...
sampDist.NPcond <- function(saemixObject,nsamp, eta.sampc=NULL, population=TRUE) {
  ...
  if(length(ismodel.struct)>0) {
    ...
    for(ityp in ismodel.struct) {
      eps.est<-saemixObject@results@iwres[saemixObject@data@data$ytype==ityp] # Estimated epsilon_ij
      epsc<-center.eps(eps.est)
      epscorr<-epsc/sd(epsc) # Correcting empirical residuals
      eps.boot[saemixObject@data@data$ytype==ityp] <- sample(epscorr,size=length(eps.est),replace=T)
    }
  } else eps.boot<-NULL
  omega.est<-saemixObject@results@omega[saemixObject@model@indx.omega,saemixObject@model@indx.omega, drop=FALSE] # Estimated var-cov matrix
