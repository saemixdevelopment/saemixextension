###########  Choose a model for covariates and random effects simultaneously by BIC in a forward algorithm  ###########
...
#' (arXiv:1612.02405)
#' @keywords selection forward covariate

forward.procedure <- function(saemixObject, trace = TRUE) {
  ...
  covariate.model <-
    matrix(c(rep(0, nb.psi * nb.covariates)),
           ncol = nb.psi,
  ...
  if (saemixObject@model@modeltype == "structural") {
    ...
    position.new.covariate <- which(covariate.model == 0)
    cov.model <- list()
    ...
    while ((select.BIC.tot <= best.BIC) && sum(best.covariate.model)!=length(best.covariate.model)) {
      ...
      position.new.covariate <- which(best.covariate.model == 0)
    ...
    if (trace == TRUE) {
      ...
      res.summary[1, ] <-
        c("<none>", re, as.character(round(BIC.tot.step[[1]][which.min(BIC.step[[1]])], 2)))
      for (j in 2:nb.steps) {
        if ((j / 2 - floor(j / 2)) != 0) {
          re <-
      ...
      rownames(best.model[[nb.steps+1]]) <- saemix.data["name.covariates"]
