...
test_that("Log-Likelihood, AIC and BIC can be extracted", {
  ...
  expect_equal(round(as.numeric(ll_is), 2), -172.80)
  expect_equal(attr(ll_is, "df"), 7)

  AIC_is <- AIC(theo_fit)
  expect_equal(round(AIC_is, 2), 359.60)

  BIC_is <- BIC(theo_fit)
  expect_equal(round(BIC_is, 2), 362.99)

  ll_lin <- logLik(theo_fit, method = "lin")
  expect_equal(round(as.numeric(ll_lin), 2), -171.96)

  AIC_lin <- AIC(theo_fit, method = "lin")
  expect_equal(round(AIC_lin, 2), 357.92)

  BIC_lin <- BIC(theo_fit, method = "lin")
  expect_equal(round(BIC_lin, 2), 361.32)
...
test_that("Model comparison works as expected", {
  ...
  expect_warning(compare.saemix(theo_fit, theo_fit_1_cov, method = "gq"),
    "not available for model 1")
  theo_fit_gq <- llgq.saemix(theo_fit)

  expect_warning(compare.saemix(theo_fit_gq, theo_fit_1_cov, method = "gq"),
