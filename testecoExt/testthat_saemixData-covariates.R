...
test_that("Successful creation of a SaemixData object, several occasions", {
  PD1.saemix<-read.table(file.path(datDir,"PD1.saemix.tab"),header=T,na=".")
  PD2.saemix<-read.table(file.path(datDir,"PD2.saemix.tab"),header=T,na=".")
  tab1<-PD1.saemix
  tab2<-PD2.saemix
  ...
  tab3$missing[c(1,4,7)]<-1
  tab3<-tab3
# Works in another testfile
#  x<-saemixData(name.data=datanocov, name.predictors=c("time","dose"), name.response="y", name.ytype="ytype",varlevel=c("id","occ")) 
# Why does this fail ?? subject and occas are valid columns
  x<-saemixData(name.data=tab3,varlevel=c("subject","occas"), name.ytype="type.rep",name.cens="censored",name.mdv="missing",name.covariates=c("gender"), verbose=F)
# Also fails when given everything explicitly... 
#  x<-saemixData(name.data=tab3,varlevel=c("subject","occas"), name.predictors = "dose", name.response="response",name.ytype="type.rep",name.cens="censored",name.mdv="missing",name.covariates=c("gender"), verbose=F)
  expect_is(x, "SaemixData") # tests for particular class
  expect_equal(x@name.predictors,c("dose"))
  expect_equal(x@name.group,c("subject"))
  expect_equal(x@name.response,"response")
