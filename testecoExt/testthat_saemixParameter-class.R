...
test_that("Defining parameters with different distributions", {
  ...
  expect_equal(ka@transform,exp)
  expect_equal(ka@transform(ka@mu.init),exp(1))
  expect_equal(vd@invtransform,log)
  expect_equal(vd@invtransform(vd@mu.init),log(10))
  expect_equal(biodisp@invtransform(biodisp@mu.init),log(0.8/.2))
  expect_equal(biodisp@transform(0),0.5)
})
...
test_that("Defining parameters with covariate effects", {
  ...
  vd<-new(Class="SaemixParameter", mu.init=10, covariate="wt", varlevel=c("iiv","iov"))
  ...
  expect_equal(vd@varlevel,c("iiv","iov"))
...
test_that("Defining parameters without IIV", {
  ...
  expect_equal(ka@transform(ka@mu.init),exp(1))
  expect_equal(vd@transform,exp)
  expect_equal(vd@varlevel,"iiv")
  expect_equal(vd@covariate.varlevel,rep("iiv",2))
})
...
test_that("Testing matching levels in list to a vector", {
  ...
  expect_equal(xvec$logmsg,"Matching elements in alist to varlevels\n")
})
