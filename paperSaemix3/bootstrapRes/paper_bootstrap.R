...
# Sections to run (toggle to TRUE to run specific chunks)
...
#################################################### count data: rapi - hurdle model

if(runmodelCountCovHurdle) {
  data(rapi.saemix) 
  rapi.saemix$gender <- ifelse(rapi.saemix$gender=="Men",1,0)  # Female=reference class as in Atkins
  ...
  # simulating from a truncated Poisson
...
# truncated Poisson component  
  case.ordinal <- saemix.bootstrap(hurdlefit1, method="case", nboot=nboot) 
  write.table(case.ordinal,file.path(workDir, "bootstrapCase_rapiHurdle1.res"), row.names = FALSE, quote=F)
  cond.ordinal <- saemix.bootstrap(hurdlefit1, method="conditional", nboot=nboot) 
  write.table(cond.ordinal,file.path(workDir, "bootstrapCond_rapiHurdle1.res"), row.names = FALSE, quote=F)
# binary component
  case.ordinal <- saemix.bootstrap(hurdlefit0, method="case", nboot=nboot) 
  write.table(case.ordinal,file.path(workDir, "bootstrapCase_rapiHurdle0.res"), row.names = FALSE, quote=F)
  cond.ordinal <- saemix.bootstrap(hurdlefit0, method="conditional", nboot=nboot) 
  write.table(cond.ordinal,file.path(workDir, "bootstrapCond_rapiHurdle0.res"), row.names = FALSE, quote=F)
