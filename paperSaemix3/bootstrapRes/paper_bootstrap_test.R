# Test file for bootstraps on discrete data for 3.2 (nboot=5 only)

library(saemix)
saemixDir<-"/home/eco/work/saemix/saemixextension"
workDir<-file.path(saemixDir,"paperSaemix3","testRes")
nboot <- 5 
...
# categorical data (knee), without and with covariates
runmodelCat <- FALSE
runmodelCatCov <- TRUE

# count data (rapi), with covariates
runmodelCountCov <- FALSE
...
if(runmodelBin) {
  ...
  write.table(case.bin,file.path(workDir, "bootstrapTestCase_binaryCov.res"), row.names = FALSE, quote=F)
  cond.bin <- saemix.bootstrap(binary.fit, method="conditional", nboot=nboot) 
  write.table(cond.bin,file.path(workDir, "bootstrapTestCond_binaryCov.res"), row.names = FALSE, quote=F)
}
...
if(runmodelCat) {
    ...
    write.table(case.ordinal,file.path(workDir, "bootstrapTestCase_knee.res"), row.names = FALSE, quote=F)
    cond.ordinal <- saemix.bootstrap(ord.fit, method="conditional", nboot=nboot) 
    write.table(cond.ordinal,file.path(workDir, "bootstrapTestCond_knee.res"), row.names = FALSE, quote=F)
    }
if(runmodelCatCov) {
    ...
    write.table(case.ordinal,file.path(workDir, "bootstrapTestCase_kneeCov.res"), row.names = FALSE, quote=F)
    cond.ordinal <- saemix.bootstrap(ord.fit.cov, method="conditional", nboot=nboot) 
    write.table(cond.ordinal,file.path(workDir, "bootstrapTestCond_kneeCov.res"), row.names = FALSE, quote=F)
}
########################################################################################################
#################################################### count data: rapi
...
if(runmodelCountCov) {
  ...
  write.table(case.ordinal,file.path(workDir, "bootstrapTestCase_rapiCov.res"), row.names = FALSE, quote=F)
  cond.ordinal <- saemix.bootstrap(zippoisson.fit.cov2, method="conditional", nboot=nboot) 
  write.table(cond.ordinal,file.path(workDir, "bootstrapTestCond_rapiCov.res"), row.names = FALSE, quote=F)
}
...
if(runbasemodelTTE) {
  ...
  write.table(case.TTE,file.path(workDir, "bootstrapTestCase_weibullTTE.res"), row.names = FALSE, quote=F)
  cond.TTE <- saemix.bootstrap(weibtte.fit, method = "conditional", nboot=nboot)
  write.table(cond.TTE,file.path(workDir, "bootstrapTestCond_weibullTTE.res"), row.names = FALSE, quote=F)
}
...
if(runcovmodelContTTE) {
  ...
  write.table(case.TTE,file.path(workDir, "bootstrapTestCase_weibullTTEcont.res"), row.names = FALSE, quote=F)
  cond.TTE <- saemix.bootstrap(weibull.fit.cov, method = "conditional", nboot=nboot)
  write.table(cond.TTE,file.path(workDir, "bootstrapTestCond_weibullTTEcont.res"), row.names = FALSE, quote=F)
}
...
if(runcovmodelCatTTE) {
  ...
  write.table(case.TTE,file.path(workDir, "bootstrapTestCase_weibullTTEcat.res"), row.names = FALSE, quote=F)
  cond.TTE <- saemix.bootstrap(weibcov.fit2, method = "conditional", nboot=nboot)
  write.table(cond.TTE,file.path(workDir, "bootstrapTestCond_weibullTTEcat.res"), row.names = FALSE, quote=F)
}
