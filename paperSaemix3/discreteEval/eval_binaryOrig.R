...
saemix.model.true<-saemixModel(model=binary.model,description="Binary model",modeltype="likelihood",
                          ...
                          transform.par=c(0,0), covariate.model=c(0,1),covariance.model=matrix(c(1,0,0,0),ncol=2), omega.init=diag(c(par.om**2,0.5)))
...
saemix.model.pop<-saemixModel(model=binary.model,description="Binary model",modeltype="likelihood",
                               ...
                               transform.par=c(0,0), covariate.model=c(0,1),covariance.model=matrix(c(1,0,0,0),ncol=2), omega.init=diag(c(1,0.5)))
...
saemix.model.far<-saemixModel(model=binary.model,description="Binary model",modeltype="likelihood",
                              ...
                              transform.par=c(0,0), covariate.model=c(0,1),covariance.model=matrix(c(1,0,0,0),ncol=2), omega.init=diag(c(4,0.5)))
## options
...
namcol <- c("irep", "theta1", "theta2", "beta", "omega1", "LL.IS")
...
for(isim in 1:nsim) {
  ...
  psi1<-data.frame(th1=rnorm(nsuj, mean=par.fix[1], sd=par.om), th2=par.fix[2])
  ...
  l1<-c(isim, smxfit.true@results@fixed.effects, sqrt(smxfit.true@results@omega[1,1]), smxfit.true@results@ll.is)
  l2<-c(isim, smxfit.pop@results@fixed.effects, sqrt(smxfit.pop@results@omega[1,1]), smxfit.pop@results@ll.is)
  l3<-c(isim, smxfit.far@results@fixed.effects, sqrt(smxfit.far@results@omega[1,1]), smxfit.far@results@ll.is)
...
# Predicted FIM using the last iteration of saemix.fit
...
model <- Model$new(
  parameter_function = function(mu, b) list(base=mu[1]+b[1], slp=mu[2], eff=mu[3]),
  ...
  omega = saemix.fit@results@omega[1,1])
