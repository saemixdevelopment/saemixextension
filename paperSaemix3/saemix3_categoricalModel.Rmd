...
The usual plot of the data object is not very informative as it alternates between 0 and 1's. Instead we plot the evolution of the frequency of infection over time in the population, stratifying by treatment. We use the *plotDiscreteData()* function from the package, setting the *outcome* argument to *'binary'*, and stratifying on *'treatment'* through the *which.cov* argument.
...
We can also present the results only for the proportion of events by visits directly on the *toenail.saemix* dataframe, using **ggplot2**, or as a barplot of the proportion of events.
...
We can also extract dataframe with the simulated data (*binary.fit@sim.data@datasim*) and produce diagnostics in R. Below, we add columns *visit* and *treatment* to plot the frequency of infection over time for each treatment.
...
The following R code represents the data as barplots of the different pain scores as a function of time in study, also illustrating recovery as the proportion of lower pain scores increases. Other graphs can be created to explore this data, such as the evolution of mean scores over time.
...
```
We then applied the stepwise algorithm proposed by Delattre et al. to simultaneously investigate the covariate model and variability structure. This was run outside Rstudio because of memory issues. The resulting model has variability on all the $\alpha$ parameters, and included treatment on $\alpha_2$ and $\beta$ and age squared on $\alpha_1$. Comparing the two models through the *compare.saemix()* function shows an improvement in the BIC of about 30 points.
...
We can also look at the VPC for the mean score in each treatment group to find that the model tends to underpredict the pain scores, especially in the group receiving therapy 1.
...
par.estim<-format(c(ord.fit.cov@results@fixed.effects,sqrt(diag(ord.fit.cov@results@omega)[ord.fit.cov@results@indx.omega])), digits=1, nsmall=1)
df2<-data.frame(parameter=colnames(case.ordinal)[-c(1)], saemix=par.estim)
for(i in 1:2) {
  if(i==1) {
    resboot1<-case.ordinal
  ...
  quant.bootDist<-apply(resboot1[-c(1)], 2, quantile, c(0.025, 0.975), na.rm=T)
  l1<-paste0(format(mean.bootDist, digits=2)," (",format(sd.bootDist,digits=2, trim=T),")")
  l2<-paste0("[",format(quant.bootDist[1,], digits=2),", ",format(quant.bootDist[2,],digits=1, trim=T),"]")
  df2<-cbind(df2, l1, l2)
  i1<-3+2*(i-1)
  colnames(df2)[i1:(i1+1)]<-paste0(namboot,".",c("estimate","CI"))
}
