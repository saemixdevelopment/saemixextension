...
- code for different bootstraps in non-linear mixed effect models (Comets et al. 2021 and submitted)
  - in the code, the number of bootstraps is set to 10 for speed but we recommend to use at least 200
...
if(nboot<200) cat("The number of bootstrap samples is too low to provide good estimates of the confidence intervals\n") else {
  ...
  df<-data.frame(Param=unique(ypd2$Param), mean.boot=mean.bootDist, est.saemix=par.estim) 
...
if(FALSE) cond.bin <- saemix.bootstrap(binary.fit, method="conditional", nboot=nboot) 
...
Here we produce a table showing tbe parameters estimated on the original dataset along with the bootstrap estimates (mean (SD) of the bootstrap distribution) and the 95% CI.
...
if(nboot<200) cat("The number of bootstrap samples is too low to provide good estimates of the confidence intervals\n") else {
  ...
  for(i in 1:2) {
    ...
    mean.bootDist<-apply(resboot1, 2, mean)[-c(1)]
    sd.bootDist<-apply(resboot1, 2, mean)[-c(1)]
...
\end{split}
\end{equation}
where $\alpha_1$ and $\beta$ are assumed to have interindividual variability and to follow a normal distribution. $\beta$ is the effect of time, $\alpha_1$ is the probability of a pain score of 1 and the other parameters represent an incremental risk to move into the higher pain category.
...
# Fitting
saemix.options<-list(seed=632545,save=FALSE,save.graphs=FALSE, nb.chains=10, nbiter.saemix=c(600,100))
...
We can then fit different models. Here we considered covariates on the two parameters with interindividual variability, first testing all covariates then reducing to a model with Age2 influencing $\alpha_1$ and treatment affecting the slope $\beta$, which had the lowest BICc, as shown using the *compare.saemix()* function.
...
**Comets E**, Rodrigues C, Jullien V, Ursino M (2021). 
