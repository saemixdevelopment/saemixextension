...
x <- cut(obsmat$y, breaks=mybreaks, include.lowest = TRUE, right=FALSE)
...
ysim.tab <- data.frame(irep=rep(1:nsim, each=dim(ydat@data)[1]), x=rep(obsmat$x, nsim), covariate.group=rep(obsmat$covariate.group, nsim), score.group=cut(ysim, breaks=mybreaks, include.lowest = TRUE, right=FALSE))
sim.scores <- ysim.tab %>%
