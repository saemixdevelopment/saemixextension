...
for(irep in 1:nsim) {
  ...
  xtab1 <- xtab %>%
    group_by(time, gender) %>%
    summarise(nev = sum(1-ysim), n=n()) %>%  # Here we want the proportion of zeroes, not 1's !
    mutate(freq = nev/n)
  )
  ytab<-rbind(ytab,xtab1[,c("time","gender","freq")])
}
...
```
We can also produce VPC for the counts as previously for the ZIP model. **TODO** bug for count 1 in hurdle model but unclear why, check.
...
# cond.count <- read.table(file.path(saemixDir,"bootstrap","results","rapi_condBootstrap.res"), header=T)
 ...
 cond.count <- read.table(file.path(saemixDir,"paperSaemix3","bootstrapCond_rapiCov.res"), header=T)
