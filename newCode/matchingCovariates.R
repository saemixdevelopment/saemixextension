...
# Model parameters
lpar <- list(ka=saemixPar(mu.start=pkpd.psi0[1], omega.start=0.3, covariate=c(sex=binCov(beta=0.3))),
             cl=saemixPar(mu.start=pkpd.psi0[2], omega.start=0.5, covariate=c(sex=binCov(beta=0.2), lwt=contCov(name="wt",transform=log, beta=0.75, beta.fix=1), lage=contCov(name="age", transform=log, beta=-0.5))),
             vd=saemixPar(mu.start=pkpd.psi0[3], omega.start=0.5, covariate=c(lwt=contCov(name="wt",transform=log, beta=1, beta.fix=1), cyp=binCov(beta=0.5)), rho.param=c("cl"), rho=0.5),
             ic50=saemixPar(mu.start=pkpd.psi0[4], covariate=c(lage=contCov(name="age", transform=log, beta=0.5), trt=catCov(beta=c(0.1,0.3)))))
npar<-length(lpar)
...
# Generate phi_i and phi_ik adding etas
...
pkpd.dat <- data.frame(id=id.suj, xidep, occ=rep(1:3, each=length(sampleTimes)*2), cov1, cov2)
...
########################
## One variability level
LCOV<-matrix(data=0, nrow=npar+sum(nb.betas), ncol=npar)
fixedpar<-c()
i1<-1
index.mu<-index.mubeta.fix<-c()
for(ipar in 1:npar) {
  fixedpar<-c(fixedpar,pkpd.psi0[ipar])
  index.mu<-c(index.mu, i1)
  LCOV[i1,ipar]<-1
  i1<-i1+1
  betas<-c()
  for(jcov in 1:length(lpar[[ipar]]@covariate)) {
    if(lpar[[ipar]]@covariate[[jcov]]@beta.fix==1) index.mubeta.fix<-c(index.mubeta.fix, i1)
    betas<-c(betas, lpar[[ipar]]@covariate[[jcov]]@beta)
    LCOV[i1,ipar]<-1
    i1<-i1+1
  }
  fixedpar<-c(fixedpar, betas)
}
########################
## More than 1 var level
### generate LCOV, Mcovariates
### First level with mu, other levels only betas"ID"
nb.betas<-unlist(lapply(covariate.model, sum))
LCOV<-Mcovariates<-vector(mode="list", length=nvarlevel)
LCOV[[1]]<-matrix(data=0, nrow=npar+nb.betas[1], ncol=npar)
for(ivar in 2:nvarlevel)
  LCOV[[ivar]]<-matrix(data=0, nrow=nb.betas[ivar], ncol=npar)
for(ivar in 1:nvarlevel)
  colnames(LCOV[[ivar]])<-names(lpar)
  
index.mu<-index.mubeta.fix<-fixedphi<-vector(mode="list", length=nvarlevel) # only index.mu of first level is non-null
psi.init<-c()
for(ipar in 1:npar) psi.init<-c(psi.init,lpar[[ipar]]@mu)
phi.init<-c(transpsi(t(psi.init),transform.par))
ivar<-1
for(ivar in 1:nvarlevel) {
  i1<-i2<-1
  xmatcov<-NULL
  covmodel<-covariate.model[[ivar]]
  for(ipar in 1:npar) {
    if(ivar==1) {
      fixedphi[[ivar]]<-c(fixedphi[[ivar]],phi.init[ipar])
      index.mu[[ivar]]<-c(index.mu[[ivar]], i1)
      if(lpar[[ipar]]@mu.fix==1) index.mubeta.fix[[ivar]]<-c(index.mubeta.fix[[ivar]], i1)
      LCOV[[ivar]][i1,ipar]<-1
      xmatcov<-cbind(xmatcov, id=rep(1,nsuj)) # Mcovariates
      i1<-i1+1
    }
    betas<-c()
    for(jcov in 1:dim(covmodel)[1]) {
      if(covmodel[jcov,ipar]==1) {"ID"
        LCOV[[ivar]][i1,ipar]<-1
        betas<-c(betas,betas.start[[ivar]][i2])
        if(betas.fix[[ivar]][i2]==1) index.mubeta.fix[[ivar]]<-c(index.mubeta.fix[[ivar]], i1)
        xmatcov<-cbind(xmatcov,covmat[[ivar]][,name.covbetas[[ivar]][i2]])
        i1<-i1+1
        i2<-i2+1
      }
    }
    if(length(betas)>0) fixedphi[[ivar]]<-c(fixedphi[[ivar]], betas)
  }
  Mcovariates[[ivar]]<-xmatcov
}
MCOV<-LCOV
for(ivar in 1:nvarlevel) {
  MCOV[[ivar]][LCOV[[ivar]]==1]<-fixedphi[[ivar]]
}
fixedpar<-fixedphi
fixedpar[[1]][index.mu[[1]]]<-c(transphi(t(fixedpar[[1]][index.mu[[1]]]), transform.par))

# Generating subject level individual parameters
## By hand
mean.fixedphi <- transpsi(matrix(pkpd.psi0,nrow=1), rep(1,4))
phi.id<-do.call(rbind, rep(list(mean.fixedphi), nsuj))
phi.id[,1]<-phi.id[,1] + lpar$ka@covariate$sex@beta * covmat.id$sex
phi.id[,2]<-phi.id[,2] + lpar$cl@covariate$sex@beta * covmat.id$sex + lpar$cl@covariate$lage@beta * covmat.id$lage
phi.id[,3]<-phi.id[,3] + lpar$vd@covariate$cyp@beta * covmat.id$cyp
phi.id[,4]<-phi.id[,4] + lpar$ic50@covariate$lage@beta * covmat.id$lage
## MCOV.id
phi.MCOV <- Mcovariates[[1]] %*% MCOV[[1]] 
summary(phi.id - phi.MCOV)

# Generating occasion level parameters
## by hand
phi1<-cbind(phi.id,id=1:nsuj)
phi.occ<-do.call(rbind, rep(list(phi1), each=nocc))
phi.occ<-cbind(phi.occ, occ=rep(1:nocc, each=dim(phi1)[1]))
phi.occ<-as.data.frame(phi.occ)
phi.occ<-phi.occ[order(phi.occ$id, phi.occ$occ),]
phi.occ$idocc <- paste(phi.occ$id, phi.occ$occ,sep="-")
...
## MCOV.occ # without column of 1 (already in MCOV.id)
Mcovariates[[2]] %*% MCOV[[2]] 

# Next step
## generate phi_i and phi_ik adding omega
## put everything in a structure (SaemixIndividualModel ?)
## see what else is needed for computations - especially optimising fixed effects ? parameters without IIV ?
...
# PK/PD design
sampleTimes<-c(0,1,2,4,8)
xidep<-data.frame(tim=sampleTimes, dose=100)
...
########################################################### Covariates for several levels of variability
# More than one level of variability
