...
estep<-function(kiter, Uargs, Dargs, opt, mean.phi, varList, DYF, phiM) {
	...
	chol.rho <- chol.omega*sqrt(varList$rho) # chol(a*A)=chol(A)*sqrt(a)
	...
	phiMc<-phiM # phiM3: copy of individual parameters 3rd kernel
	...
	# Third stage - changed to random walk from phiM with rho*Omega
	if(opt$nbiter.mcmc[3]>0) {
	  nt2<-opt$nbiter.mcmc[3]*Dargs$NM
	  nbc2<-0
	  for (u in 1:opt$nbiter.mcmc[3]) {
	    etaMc<-matrix(rnorm(Dargs$NM*nb.etas),ncol=nb.etas)%*%chol.rho
	    phiMc[,varList$ind.eta]<-phiM[,varList$ind.eta,drop=FALSE]+etaMc
	    Uc.y<-compute.LLy(phiMc,Uargs,Dargs,DYF,varList$pres)
	    deltau<-Uc.y-U.y
	    ind<-which(deltau<(-1)*log(runif(Dargs$NM)))
	    etaM[ind,]<-phiMc[ind,varList$ind.eta,drop=FALSE]-mean.phiM[ind,varList$ind.eta,drop=FALSE]
	    phiM[ind,] <- phiMc[ind,] # update phiM ? (not sure)
	    U.y[ind]<-Uc.y[ind]
	    nbc2<-nbc2+length(ind)
	  }
	  varList$rho<-varList$rho*(1+opt$stepsize.rw* (nbc2/nt2-opt$proba.mcmc)) 	   
#	  cat(rho,"...")
	  # not sure about this step... trying to adjust rho to maintain stg close to proba.mcmc
	  # alternatively can update directly domega2 ?
#	  varList$domega2[,varList$ind.eta]<-varList$domega2[,varList$ind.eta]*(1+opt$stepsize.rw* (nbc2/nt2-opt$proba.mcmc))
	}
