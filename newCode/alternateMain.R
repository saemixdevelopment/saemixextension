...
saemixAlternate<-function(model,data,control=list()) {
...
# after SA - resetting sufficient statistics
#    if(opt$flag.fmin && kiter==saemix.options$nbiter.sa) {
    # after K1 - resetting sufficient statistics and reducing IIV
    if(opt$flag.fmin && kiter==saemix.options$nbiter.saemix[1]) {
...
############# Stochastic Approximation
  	xstoch<-mstepAlternate(kiter, Uargs, Dargs, opt, structural.model, DYF, phiM, varList, phi, betas, suffStat)
