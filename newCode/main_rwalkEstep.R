...
estep<-function(kiter, Uargs, Dargs, opt, mean.phi, varList, DYF, phiM) {
	...
	# Third stage - changed to random walk from phiM with rho*Omega=domega2
	if(opt$nbiter.mcmc[3]>0) {
	  nt2<-opt$nbiter.mcmc[3]*Dargs$NM
	  nbc2<-0
	  for (u in 1:opt$nbiter.mcmc[3]) {
	    etaMc<-etaM+matrix(rnorm(Dargs$NM*nb.etas), ncol=nb.etas)%*%varList$domega2 
	    phiMc[,varList$ind.eta]<-mean.phiM[,varList$ind.eta]+etaMc
	    Uc.y<-compute.LLy(phiMc,Uargs,Dargs,DYF,varList$pres)
	    deltau<-Uc.y-U.y
	    ind<-which(deltau<(-1)*log(runif(Dargs$NM)))
	  ...
	  varList$domega2<-varList$domega2*(1+opt$stepsize.rw* (nbc2/nt2-opt$proba.mcmc))
	}
