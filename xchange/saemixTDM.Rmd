...
We note that in the last setting we cannot recover the simulated value of the EC50, even for the subject with a rich design (although there is a stronger signal for this subject). This is due to the low value of the estimated variabiliy on EC50 (only about 18%), since the conditional distribution depends both on the observed data and on the prior distribution represented by the population estimates.
