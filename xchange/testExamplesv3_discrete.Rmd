...
if(testMode) {
  dev_mode()
  install.packages(file.path(versionDir,"saemix3.1","saemix_3.1.tar.gz"),repos=NULL)
  library(saemix)
}
```
...
nsim<-100
yfit1<-simulateDiscreteSaemix(poisson.fit.cov2, saemix.simulatePoisson, nsim=nsim)
...
yfit2<-simulateDiscreteSaemix(zippoisson.fit.cov2, saemix.simulatePoissonZIP, 100)
...
nsim<-100
ysim.hurdle0 <- simulateDiscreteSaemix(hurdlefit0, simulBinary, nsim=nsim)
...
Plot evolution of median score ? VPC on selected scores or on categories (more complicated and not saemix, more using R code with the simulated data)
