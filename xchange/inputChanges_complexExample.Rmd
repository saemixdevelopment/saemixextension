...
**Read more complex data and create more complex models for next version (4.0)**

**TODO Eco: simulation with covariates, simulation with missing data**
...
datacov <- read.csv(file=file.path(datDir,"complexSimulation_cov.csv"), header=T, sep=",")
...
```{r readDataCov, results='hide', echo=FALSE, warnings=FALSE}
# Reading data into an saemixData object
smxdata.cov <- saemixData(name.data=file.path(datDir,"complexSimulation_cov.csv"), header=T,na=".", sep=",",
      name.group=c("id"),name.predictors=c("time", "dose"),name.response=c("y"), name.occ = "occ", name.ytype = "ytype", 
      name.covariates=c("sex","age","wt","comed","crcl"),
      units=list(x="weeks",covariates=c("-","yr","kg","-","ml/min/1.73m2 BSA")),verbose=TRUE)
xcov<-smxdata.cov
xdatcov <- smxdata.cov@data
summary(xdatcov)
```
...
for(islot in object.slots) {
  if(!(islot %in% c("outcome", "sep", "na", "header","class","units","log","varlevel","covariate.varlevel"))) {
...
### Models used to simulate data

- Model used to simulate responses
  - response
