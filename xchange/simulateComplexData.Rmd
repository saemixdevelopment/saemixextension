...
- Features
  ...
  - design
    ...
    - dose: D=4mg
    - time between two occasions: 4 weeks (assume washout)
- Model
  ...
  - variabilities
    ...
    - combined residual error model with a=0.1 and b=0.1
- Covariates
  - baseline covariates
    - equal number of each gender
    - weight: normal distribution centering on different values for men (70) and women (60) with the same coefficient of variation (20%)
  - covariate changing with occasion
    - CRCL: log-normal distribution at baseline, decrease with time (on average 10% per month, ~N(0.1, 0.05))
...
### Simulating individual parameters with and without covariates

```{r simulateTheoIOVindivpar}
#################### Design
nsuj <- 100
sampleTimes <- c(0.25, 1.5, 3.35, 12, 24)
...
# Parameters
doses <- c(4)
param <- data.frame(ka=1.48, V=0.48, CL=0.04, betaCRCL=0.5)
## IIV
omega.iiv <- rep(0.3,3)
varcovmat<-diag(c(omega.iiv[2],omega.iiv[3])**2) # correlation between delta and CL
varcovmat[1,2]<-varcovmat[2,1]<-0.7*omega.iiv[2]*omega.iiv[3]
## IOV
omega.iov <- c(0.25)
...
cov.base <- data.frame(id=1:nsuj, sex=1, wt = trunc(rnorm(nsuj, mean=60, sd=12)))
...
### CLCR at baseline ~ U(min-max)
### normal values: 97 to 137 mL/min for men and 88 to 128 mL/min for women
#### Decrease of 5% (~N(0.05, 0.01)) every month for the purpose of the exercise
...
for(i in 1:nsuj) {
  ...
  crcl<-trunc(runif(1,97,137))-ifelse(cov.base$sex[1]==1,9,0)
  crcl<-c(crcl, round(crcl*rnorm(1,mean=0.95,sd=0.01), digits=2))
  crcl<-c(crcl, round(crcl[2]*rnorm(1,mean=0.95,sd=0.01), digits=2))
  tabsuj<-cbind(tabsuj,crcl=crcl[1:nocc[i]], comed=rbinom(nocc[i],1,0.3))
...
etas <- matrix(rnorm(nsuj*3,mean=0,sd=1), ncol=3)
psi1$ka <- param$ka*exp(etas[,1]*omega.iiv[1])  # log-normal distribution
psi1$V <- param$V*exp(tab1[,1]) # log-normal distribution, omega.iiv already included
psi1$CL<-param$CL*exp(tab1[,2]) # log-normal distribution

psi1.nocov <- psi1

# Adding subject-specific covariates
psi1$V <- param$V*exp(tab1[,1])*(cov.base$wt/70)*(0.7*cov.base$sex+(1-cov.base$sex))
psi1$CL<-param$CL*exp(tab1[,2])*(cov.base$wt/70)^0.75 # log-normal distribution
psi1.cov <- psi1
...
psi1.iov<-NULL
for(isuj in 1:nsuj) {
  psisuj <- do.call(rbind,rep(list(psi1.nocov[isuj,]),nocc[isuj]))
  psisuj$occ<-1:nocc[isuj]
#  eta.iov<-rmvnorm(nocc[isuj],sigma=iovmat)
  eta.iov<-rnorm(nocc[isuj],mean=0,sd=omega.iov)
#  psisuj$CL <-psisuj$CL*exp(eta.iov[,2])
  psisuj$CL <-psisuj$CL*exp(eta.iov)
  psi1.iov<-rbind(psi1.iov, psisuj)
}
psi1.iov$index <- paste0(psi1.iov$id,".",psi1.iov$occ)

psi1.iov.nocov <- psi1.iov

## Adding occasion-specific covariates
psi1.iov<-NULL
for(isuj in 1:nsuj) {
  psisuj <- do.call(rbind,rep(list(psi1.cov[isuj,]),nocc[isuj]))
  psisuj$occ<-1:nocc[isuj]
  #  eta.iov<-rmvnorm(nocc[isuj],sigma=iovmat)
  eta.iov<-rnorm(nocc[isuj],mean=0,sd=omega.iov)
  #  psisuj$CL <-psisuj$CL*exp(eta.iov[,2])
...
psi1.iov$CL <- psi1.iov$CL*(cov.occ$comed*1.3+(1-cov.occ$comed))*(cov.occ$crcl/110)^(param$betaCRCL)
psi1.iov$index <- paste0(psi1.iov$id,".",psi1.iov$occ)

psi1.iov.cov <- psi1.iov
```

### Simulating responses

```{r simulateTheoIOVresp}
# Predicted data (same times for each occasion)
xidep1 <- data.frame(tim=rep(sampleTimes,dim(psi1.iov.nocov)[1]), dose=doses[1], matchidx=rep(psi1.iov.nocov$index, each=length(sampleTimes)))
id1<-match(xidep1$matchidx,psi1.iov.nocov$index)
mat1 <- psi1.iov.nocov$index[match(xidep1$matchidx,psi1.iov.nocov$index)]
mat1 <- matrix(unlist(strsplit(mat1,".",fixed=T)), ncol=2, byrow=T)


# Simulated data (adding error)
## No covariate
psi1 <- as.matrix(psi1.iov.nocov[,2:4])
ypred <- theo1cpt(psi1,id1,xidep1)
gpred <- sqrt(par.sig[1]**2 + (ypred*par.sig[2])**2)
ntot<-length(ypred)
simdata<-data.frame(id=as.integer(mat1[,1]), tim=xidep1$tim, occ=as.integer(mat1[,2]), dose=doses[1], ypred=ypred, 
                    ysim=ypred+gpred*rnorm(ntot,0,1))

ggplot(simdata, aes(x=tim, y=ypred, colour=as.factor(occ))) + geom_point() + xlab("Time (weeks)") + ylab("Predicted concentrations")  + scale_color_viridis("Occasion",discrete = TRUE) + facet_wrap(.~occ)

datsim.nocov <- simdata

if(FALSE)
  plot(xidep1$tim, ypred)

# With covariates
psi1 <- as.matrix(psi1.iov.cov[,2:4])
ypred <- theo1cpt(psi1,id1,xidep1)
gpred <- sqrt(par.sig[1]**2 + (ypred*par.sig[2])**2)
ntot<-length(ypred)
simdata<-data.frame(id=as.integer(mat1[,1]), tim=xidep1$tim, occ=as.integer(mat1[,2]), dose=doses[1], ypred=ypred, 
...
simdata <- cbind(simdata,
                 cov.base[match(simdata$id,cov.base$id),2:3],
                 cov.occ[id1,c(6,7)])

ggplot(simdata, aes(x=tim, y=ypred, group=as.factor(sex), colour=as.factor(sex))) + geom_point() + xlab("Time (weeks)") + ylab("Predicted concentrations")  + scale_color_viridis("Sex",discrete = TRUE) + facet_wrap(.~occ)
...
if(saveSimData) write.csv(datsim.nocov, file=file.path(datDir,"theoSimul_nocov.csv"), quote=F, row.names = FALSE)

if(saveSimData) write.csv(datsim.cov, file=file.path(datDir,"theoSimul_cov.csv"), quote=F, row.names = FALSE)

if(saveSimData) write.csv(psi1.iov.nocov, file=file.path(datDir,"theoSimul_psinocov.csv"), quote=F, row.names = FALSE)

if(saveSimData) write.csv(psi1.iov.cov, file=file.path(datDir,"theoSimul_psicov.csv"), quote=F, row.names = FALSE)
...
```{r saveDataNoCov}
if(saveSimData) write.csv(psi1.iov, file=file.path(datDir,"complexSimulation_simpsiNocov.csv"), quote=F, row.names = FALSE)

if(saveSimData) write.csv(simdata, file=file.path(datDir,"complexSimulation_nocov.csv"), quote=F, row.names = FALSE)
...
- a **second dataset** is created with covariates
  - baseline covariates
    - equal number of each gender
    - weight: normal distribution at baseline, centering on different values for men (70) and women (60) with the same coefficient of variation (15%)
    - uniform distribution of age from 30 to 70
  - covariate changing with occasion
    - Serum creatinine: normal distribution at baseline, assuming 10% change every year (% change ~ N(0.1, 0.01))
    - CRCL computed with Cockroft-Gault formula
    - comedication: random allocation at each occasion, with probability 0.3
  - covariate models
    - weight: allometry on V and CL for biomarker 2
      - $CL = \mu_{CL} \; \left( \frac{WT}{WT_{ref}} \right)^{0.75}$
      - $V = \mu_{V} \; \left( \frac{WT}{WT_{ref}} \right)$
      - with WT_ref=70 kg
    - gender: decreased intercept for biomarker 1 (by 30%) and higher probability for biomarker 3 (30%)
      - model: $\alpha = \mu_{\alpha} + \beta_{sex,\alpha}*1_{sex=female}$
        - $\beta_{sex,\alpha}$=-30
      - model: $logit(p3) = logit(\mu_{p3}) + \beta_{sex,p3}*1_{sex=female}$
        - $\beta_{sex,p3}$=0.4 corresponding to p3=39% in women and 30% in male
    - age: decrease of CL with age, increase of $p_3$
      - model for CL: $CL = \mu_{CL} \; \left( \frac{age}{age_{ref}} \right)^{\beta_{age,CL}}$
        - age_ref=50, beta_age,CL=-0.2
        - corresponds to CL varying from 2.76 for age=30 to 2.33 for age=70 (~16% decrease over the range)
      - model for p3: $logit(p3) = logit(\mu_{p3}) + \beta_{age,p3}*(age-age_{ref})$
        - age_ref=50, beta_age,p3=0.024
        - corresponds to p3 varying from 0.21 for age=30 to 0.41 for age=70 (~100% increase over the range, but probability)
    - CRCL: correlation with CL
      - model: (CRCL/CRCL_ref)^beta_CRCL 
      - CRCL_ref=85, beta_CRCL=0.3
      - corresponds to CL varying from 1.95 for CRCL=37 to 2.93 for CRCL=144 (~50% increase over the range)
    - comedication: increase in CL by 30%, decrease $p_3$ by 50%
      - model: $logit(p3) = logit(\mu_{p3}) + \beta_{comed,p3}*1_{comed}$
      - beta_{comed,p3}=-0.89 corresponding to p3=15% with comedication
- Not included in the simulation
  - NA values in the covariates
...
### age ~ N(60, 9)
male.id <- sort(sample(1:nsuj, nsuj/2, replace=FALSE))
cov.base <- data.frame(id=1:nsuj, sex=1, age=trunc(runif(nsuj,30,71)), wt = trunc(rnorm(nsuj, mean=60, sd=9)))
cov.base$sex[male.id]<-0
cov.base$wt[male.id]<-trunc(rnorm(nsuj/2, mean=70, sd=10.5))
summary(cov.base)
...
## note: other sources would give males (0.7-1.2) and females (0.5-1.0) => used here
#### Scr at baseline ~ N(0.75,0.08) + 0.2*(sex=0)
...
for(i in 1:nsuj) {
  ...
  tabsuj<-cbind(tabsuj, occ=1:nocc[i])
# Scr: mg/dL
  scr<-rnorm(1, mean=0.75, sd=0.08) + 0.2*(1-cov.base$sex[i])
...
### Pb: the predicted CrCL values are lower than the normal range even though all variables have been simulated and Scr within the normal ranges... Here: 37 to 144
# Creatinine clearance, however, may over-estimate GFR by 10-20% and has largely been replaced clinically by equations to more directly estimate GFR.

# CKD-EPI equation - 2021 (ml/min for 1.73m2 BSA) for eGFR
vec1<-cov.occ$scr/ifelse(cov.occ$sex==1,0.7,0.9) # Scr/kap where kap=0.7 for females and 0.9 for males
cov.occ$egfr <- 142* (sapply(vec1,function(x) min(x,1)))^(ifelse(cov.occ$sex==1,-0.241,-0.302)) *
  (sapply(vec1,function(x) max(x,1)))^(-1.2) *0.9938^(cov.occ$age)*(1*(1-cov.occ$sex)+1.012*cov.occ$sex)
# a completely different value but closer to the normal range
# normal values: 116 (at 20) to 76 (at 70)
# here: 62 to 110
# GFR>= 90 classified as normal

beta.cov <- data.frame(beta.sexalpha=-30, beta.sexp3=0.4, beta.ageCL=-0.2, beta.agep3=0.024, beta.CRCLCL=0.3, beta.comedCL=0.3, beta.comedp3=-0.89)

# Generating parameters with covariates
psi1.cov<-NULL
for(isuj in 1:nsuj) {
  psisuj<-as.numeric(psi1[isuj,])
  covsuj <- cov.occ[cov.occ$id==isuj,,drop=FALSE]
  niocc<-covsuj$nb.occ[1]
  lp3 <- log(psisuj[7]/(1-psisuj[7]))
  logitp3 <- rep(lp3,niocc) + beta.cov$beta.sexp3*covsuj$sex + beta.cov$beta.agep3*(covsuj$age-50) + beta.cov$beta.comedp3*covsuj$comed
  psi2 <- data.frame(id=covsuj$id,alpha=psisuj[2]+beta.cov$beta.sexalpha*covsuj$sex, 
             delta=rep(psisuj[3],niocc),ka=rep(psisuj[4],niocc),
             V=psisuj[5]*(covsuj$wt/70),
             CL=psisuj[6]*(covsuj$wt/70)^0.75 * (covsuj$age/50)^(beta.cov$beta.ageCL) * (covsuj$crcl/85)^(beta.cov$beta.CRCLCL) * exp(beta.cov$beta.comedCL*covsuj$comed),
             p3=1/(1+exp(-logitp3)),
             delta.y1=rep(psisuj[8],niocc),delta.y2=rep(psisuj[9],niocc),
             occ=covsuj$occ
             )
  psi1.cov<-rbind(psi1.cov, psi2)
}

# Adding IOV
psi1.iov.cov<-NULL
for(isuj in 1:nsuj) {
  psisuj <- psi1.cov[psi1.cov$id==isuj,]
  eta.iov<-rmvnorm(nocc[isuj],sigma=iovmat)
  psisuj$delta <-psisuj$delta*(1+eta.iov[,1])
  psisuj$CL <-psisuj$CL*exp(eta.iov[,2])
  # steady increase in biomarker with time across occasions
  ...
  if(nocc[isuj]>2)
    psisuj$alpha[3]<-psisuj$alpha[2]+psisuj$delta[2]*36
  psi1.iov.cov<-rbind(psi1.iov.cov, psisuj)
}
psi1.iov.cov$index <- paste0(psi1.iov.cov$id,".",psi1.iov.cov$occ)
...
for(isuj in 1:nsuj) {
  ...
  id.occ<-paste0(rep(isuj,dim(xidep)[1]),".",xidep$occ)
  xid <- match(id.occ, psi1.iov.cov$index)
  ypred<-simul.biomarkers(psi1.iov.cov[,-c(1)], xid, xidep)
  datsuj <- data.frame(id=isuj, time=xidep$tim, dose=xidep$dose, ytype=xidep$ytype, occ=xidep$occ,ypred=round(ypred,digits=2), y=ypred)
  datsuj$y[datsuj$ytype==1] <- round(datsuj$y[datsuj$ytype==1] + par.sig[1]*rnorm(sum(datsuj$ytype==1)), digits=2)
  datsuj$y[datsuj$ytype==2] <- round(datsuj$y[datsuj$ytype==2]*(1 + par.sig[2]*rnorm(sum(datsuj$ytype==2))), digits=2)
  simdata <- rbind(simdata, datsuj)
...
isuj <- unique(simdata$id[simdata$occ==3])[1]
datsuj<-simdata[simdata$id==isuj,]
ggplot(datsuj, aes(x=time, y=y, group=occ, colour=as.factor(occ))) + geom_point(aes(shape=as.factor(ytype))) + geom_line(data = subset(datsuj, ytype<3)) + xlab("Time (weeks)") + ylab("Responses")  + scale_color_viridis("Occasion",discrete = TRUE) + scale_shape(name = "Response") + facet_wrap(.~ytype, scales="free_y")

# par(mfrow=c(1,3)); for(ity in 1:3) plot(datsuj$time[datsuj$ytype==ity], datsuj$y[datsuj$ytype==ity], type="b")
...
bio3 <- NULL
for(iocc in 1:3) {
  datocc <- simdata[simdata$ytype==3 & simdata$occ==iocc,]
  xtim<-sort(unique(datocc$time))
  for(itim in xtim) {
    y3<-datocc$y[datocc$time==itim]
    bio3<-rbind(bio3,c(iocc,itim,length(y3),sum(y3)/length(y3)))
  }
}
bio3<-as.data.frame(bio3)
colnames(bio3)<-c("occ","time","nobs","p")
bio3$sd <- sqrt(bio3$p*(1-bio3$p)/bio3$nobs)
bio3$lower <- bio3$p -1.96*bio3$sd
bio3$upper <- bio3$p +1.96*bio3$sd
ggplot(data=bio3, aes(x=time, y=p, group=as.factor(occ), colour=as.factor(occ),fill=as.factor(occ))) + geom_point() + geom_line() + geom_ribbon(data=bio3,aes(ymin=lower, ymax=upper), alpha=0.2) + xlab("Time (weeks)") + ylab("Proportion of events")  + scale_color_viridis("Occasion",discrete = TRUE) + scale_fill_viridis("Occasion",discrete = TRUE)

# Generating responses
## psi
if(saveSimData) write.csv(psi1.iov.cov, file=file.path(datDir,"complexSimulation_simpsiCov.csv"), quote=F, row.names = FALSE)

## simulated data, with covariates
lab1 <- paste0(cov.occ$id,".", cov.occ$occ)
lab2 <- paste0(simdata$id,".", simdata$occ)
simdataCov <- cbind(simdata, cov.occ[match(lab2,lab1),c(5,2:4,7:9)])
if(saveSimData) write.csv(simdataCov, file=file.path(datDir,"complexSimulation_cov.csv"), quote=F, row.names = FALSE)
