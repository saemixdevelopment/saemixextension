...
### SaemixData object - CRAN version

```{r createSaemixData}

```

### SaemixModel object - CRAN version without IOV

```{r createLegacySaemixModel}
...
for(islot in object.slots) {
  if(!(islot %in% c("outcome", "sep", "na", "header","class","log","varlevel"))) {
...
smxmodel.legacy<-saemixModel.legacy(model=theo1cpt, modeltype=c("structural"),
  description="Theophylline PK model without IOV", 
  psi0=psi0.mat,transform.par=rep(1,3),covariance.model=varmat,error.model=c("combined"))
if(smxmodel.legacy=="Creation of SaemixModel failed") cat("ToDo: code a legacy function to recognise the old instructions\n")
