...
#' @slot fim the Fisher information matrix - expected, obtained by linearisation
...
#' @slot fim.sa the Fisher information matrix - observed, obtained by stochastic approximation
#' @slot se.fixed.sa  a vector giving the estimated standard errors of estimation for the fixed effect parameters
#' @slot se.omega.sa a vector giving the estimated standard errors of estimation for Omega 
#' @slot se.cov.sa a matrix giving the estimated SE for each term of the covariance matrix (diagonal elements represent the SE on the variances of the random effects and off-diagonal elements represent the SE on the covariance terms)
#' @slot se.respar.sa a vector giving the estimated standard errors of estimation for the parameters of the residual variability
#' @slot conf.int.sa a dataframe containing the estimated parameters, their estimation error (SE), coefficient of variation (CV), and the associated confidence intervals; the variabilities for the random effects are presented first as estimated (variances) then converted to standard deviations (SD), and the correlations are computed. For SD and correlations, the SE are estimated via the delta-method
...
setClass(
  ...
  representation=representation(
    ...
    fim="matrix",		# Fisher information matrix - expected, obtained by linearisation
    ...
    fim.sa="matrix", # Fisher information matrix - observed, obtained by stochastic approximation
    se.fixed.sa="numeric",		# estimated SE for fixed effects
    se.omega.sa="numeric",	# estimated SE for Omega (diagonal terms of Omega, presented as  vector)
    se.cov.sa="matrix",	# matrix giving the estimated SE for each term of the covariance matrix (diagonal elements represent the SE on the variances of the random effects and off-diagonal elements represent the SE on the covariance terms)
    se.respar.sa="numeric",	# estimated SE for residual variability
    conf.int.sa="data.frame",   # Estimates, SE, CV and confidence intervals (assuming normality of the estimators)
...
setMethod(
  ...
  switch (EXPR=i,
    ...
    "fim.sa"={return(x@fim.sa)},
    "se.fixed.sa"={return(x@se.fixed.sa)},
    "se.omega.sa"={return(x@se.omega.sa)},
    "se.cov.sa"={return(x@se.cov.sa)},
    "se.respar.sa"={return(x@se.respar.sa)},
    "conf.int.sa"={return(x@conf.int.sa)},
...
setReplaceMethod(
  ...
  switch (EXPR=i,
    ...
    "fim.sa"={x@fim.sa<-value},
    "se.fixed.sa"={x@se.fixed.sa<-value},
    "se.omega.sa"={x@se.omega.sa<-value},
    "se.cov.sa"={x@se.cov.sa<-value},
    "se.respar.sa"={x@se.respar.sa<-value},
    "conf.int.sa"={x@conf.int.sa<-value},
