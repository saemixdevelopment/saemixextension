saemix.fim_stoch<-function(model,data,control=list()) {
  ...
  deltai = 0 # on pourrait définir dans initialiseMainAlgo mais ça implique de modifier cette fonction aussi
  task_fim.sa = saemix.options$fim.sa
  ...
  for (kiter in 1:saemix.options$nbiter.tot) { # Iterative portion of algorithm
    ...
    if(opt$stepsize[kiter]>0) {
      ...
      xstoch<-mstep.fim_stoch(kiter, Uargs, Dargs, opt, structural.model, DYF, phiM, varList, phi, betas, suffStat, deltai, covariance.model, task_fim.sa)
  ...
  if(saemix.options$fim.sa) {
    npartot = length(deltai[,1])
    indfim_stoch = array(data=NA,dim=c(npartot,npartot,Dargs$N))
    for (i in 1:Dargs$N){
      indfim_stoch_i = deltai[,i] %*% t(deltai[,i])
      indfim_stoch[,,i] = indfim_stoch_i
    }
    fim_stoch =   rowSums(indfim_stoch, dims = 2)
  }
  ...
  # Stochastic FIM & update saemix.res
  if(saemix.options$fim.sa) {
    x<-try(saemixObject<-transfFim.sa(saemixObject, fim_stoch))
    if(inherits(x,"try-error") & saemixObject@options$warnings) message("Problem transforming the stochastic FIM\n")
  }
