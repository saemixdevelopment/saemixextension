...
#' error of the estimated parameters, when all responses are continuous
#' 
...
fim.saemix<-function(saemixObject) {
  ...
  if(sum(saemix.model@modeltype != "structural")>0) {
    if(fit_sim2@options$warnings)  cat("Computing the log-likelihood and FIM via linearisation is not adequate for non Gaussian outcomes. Please use another approach (bootstrap, stochastic approximation) \n")
    return(saemixObject)
    }  
  xind<-saemix.data["data"][,c(saemix.data["name.predictors"],saemix.data["name.cens"],saemix.data["name.mdv"],saemix.data["name.ytype"])
,drop=FALSE]
...
#    } else Vi<- DF[j1:j2,,drop=FALSE] %*% t(DF[j1:j2,,drop=FALSE])+ mydiag(1, nrow=ni)
    #    invVi[[i]]<-solve(Vi[[i]])
...
#    else namallpar<-c(saemixObject@results@name.fixed,name.rand1, name.rand2)
...
#        if(ipar.res%%2 == 1) DV[[ipar]]<-mydiag(2*g0i, nrow=ni) else DV[[ipar]]<-mydiag(2*g0i*f0i, nrow=ni)
        ...
        if(ipar.res%%2 == 1) DV[[ipar]]<-mydiag(2*saemix.res@respar[ipar.res], nrow=ni) else DV[[ipar]]<-mydiag(2*saemix.res@respar[ipar.res]*f0i*f0i, nrow=ni)
      }
    }
#    }
    # for(ipar.res in 1:(2*nytype)) {
...
#    if(saemixObject@model@modeltype=="structural") 
    estpar<-c(estpar,est1,saemixObject@results@respar[saemixObject@results@indx.res],est2) #  else   estpar<-c(estpar,est1,est2)
#    if(saemixObject@model@modeltype=="structural") 
    estSE<-c(estSE,se1,se.res[saemixObject@results@indx.res],se2) # else estSE<-c(estSE,se1,se2)
  } else {
