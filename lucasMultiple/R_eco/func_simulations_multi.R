...
simulate.SaemixObject<-function(object, nsim, seed, predictions, outcome="continuous", res.var=TRUE,uncertainty=FALSE,...) {
  ...
  predictions<-(sum(predictions)>0)
  if(length(object@model@modeltype)>1) {# if more than one outcome but only continuous responses, we can use simulateContinousSaemix
    if(sum(object@model@modeltype=="likelihood")>1) { # if more than one non-continuous responses, exit
      if(object@options$warnings) cat("Currently not able to simulate more than one non-continuous outcome\n")
      return(object)
    } else if(object@options$warnings & sum(object@model@modeltype!="structural")>0) cat("Data will be simulated using simulateContinuousSaemix() and will not be valid for the non-continuous outcome\n")
  }
  if(missing(seed)) seed<-NULL
  if(!is.null(seed)) set.seed(seed)
  if(missing(nsim)) nsim<-object["options"]$nb.sim
  if(sum(object@model@modeltype=="structural")>0) object<-simulateContinuousSaemix(object=object, nsim=nsim, seed=seed, predictions=predictions, uncertainty=uncertainty, res.var=res.var, ...) else {
