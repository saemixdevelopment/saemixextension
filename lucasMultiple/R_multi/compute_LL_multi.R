...
llis.saemix<-function(saemixObject) {
  ...
  for(km in 1:KM) {
    ...
    #			f<-log(cutoff(f))
    ytype = XM$ytype
    for(itype in 1:length(saemixObject["model"]["modeltype"])) {
      if (saemixObject["model"]["modeltype"][itype]=="structural"){
        g<-error(f,pres,XM$ytype)
        DYF[ind.ioM][ytype==itype] <- -0.5*((yM[ytype==itype]-f[ytype==itype])/g[ytype==itype])**2 - log(g[ytype==itype]) - 0.5*c1
      } else {
        DYF[ind.ioM][ytype==itype] <- f[ytype==itype]
      }
    }
...
#	c1<-log(2*pi)
	...
	for (j in 1:nx) {
		...
		for(itype in 1:length(saemixObject["model"]["modeltype"])) {
		  if (saemixObject["model"]["modeltype"][itype]=="structural"){
		    DYF[ind.io][ytype==itype] <- -0.5*((yobs[ytype==itype]-f[ytype==itype])/g[ytype==itype])**2 - log(g[ytype==itype]) # - 0.5*c1
		  } else {
		    DYF[ind.io][ytype==itype] <- f[ytype==itype]
		  }
		}
		ly<-colSums(DYF)
