...
mstep.multi<-function(kiter, Uargs, Dargs, opt, structural.model, DYF, phiM, varList, phi, betas, suffStat, deltai) {
  ...
  # IL FAUDRA GERER LE EXPONENTIEL PLUS TARD STP 
  ytype = Dargs[["XM"]][["ytype"]]
  unique_type = unique(ytype)
  for(ityp in Dargs$etype.exp) fpred[Dargs$XM$ytype==unique_type[ityp]]<-log(cutoff(fpred[Dargs$XM$ytype==unique_type[ityp]]))
  ...
  for(k in 1:Uargs$nchains) {
    ...
    if(length(suffStat)>3) { # at least one response is not given by its LL
      for(itype in 1:length(Dargs$modeltype)) {
        unique_type = unique(Dargs$XM$ytype)
        if(Dargs$modeltype[itype]=="structural") {
          if(!is.na(match(Dargs$error.model[itype],c("constant","exponential"))))
            resk<-sum((Dargs$yobs[tab_ytype==unique_type[itype]]-fk[tab_ytype==unique_type[itype]])**2) else {
              resk<-sum((Dargs$yobs[tab_ytype==unique_type[itype]]-fk[tab_ytype==unique_type[itype]])**2/cutoff(fk[tab_ytype==unique_type[itype]]**2,.Machine$double.eps))
            }
...
#   #### numerical gradient 
  ...
  # A PARTIR D UNE ITERATION DE 300 ON PART SUR LE ELSE ? IL Y A UN BUG 
  ...
  for(l in 1:length(coef)){
    for(j in 1:length(varList$pres[varList$pres!=0 & is.na(varList$pres)==F])){ # les erreurs qui ne sont non nulles et NA
      w = which(varList$pres!=0 & is.na(varList$pres)==F)[j] # on récupère ces index
      pres = varList$pres
      pres[w]<-varList$pres[w]+coef[l]*d #pour chaque index de ces erreurs , soit on les mets à zéro soit nombre très faible
  ...
  omega.eta_copy = omega.eta[1:length(Uargs$i1.omega2),1:length(Uargs$i1.omega2)]
  if (!is.matrix(omega.eta_copy)){
    omega.eta_copy = matrix(omega.eta_copy)
  }
  omega = diag(omega.eta_copy) # MODIFICATION ICI J AI MIT EN MATRIX SI OMEGA A UNE VALEUR 
 ...
 # COMPORTEMENT BIZARRE AVEC l  = 2 , j_ = 2
              mcov2 = mcovTot
    ...
    for(itype in 1:length(Dargs$modeltype)) {
      if(Dargs$modeltype[itype]=="structural") {
        ...
        if (Dargs$error.model[itype] %in% c("constant","exponential")) {
          sig2<-suffStat[[i1+3]]/length(ytype[ytype==unique_type[itype]])
        ...
        if(Dargs$error.model[itype]=="combined") { ## UNTESTED; need to compute the ssq for the response itype !!!
          ...
          suppressWarnings(ABres<-abs(optim(par=varList$pres[c(1:2)+(i1-1)*2],fn=ssq,y=Dargs$yM[Dargs$XM$ytype==unique_type[itype]],f=fpred[Dargs$XM$ytype==unique_type[itype]],etype=Dargs$XM$ytype[Dargs$XM$ytype==unique_type[itype]],all_param=varList$pres,i1=i1)$par))
          #suppressWarnings(ABres<-abs(optim(par=varList$pres[c(1:2)+(i1-1)*2],fn=ssq,y=Dargs$yM,f=fpred,etype=Dargs$XM$ytype)$par))
          if (kiter<=opt$nbiter.sa) {
            varList$pres[c(1:2)+(i1-1)*2]<-c(max(varList$pres[1+(i1-1)*2]*opt$alpha1.sa,ABres[1]),
                                             max(varList$pres[2+(i1-1)*2]*opt$alpha1.sa,ABres[2]))
          }  else {
            if (kiter<=opt$nbiter.saemix[1]) {
              varList$pres[c(1:2)+(i1-1)*2]<-c(ABres[1],ABres[2])
            } else {
              varList$pres[c(1:2)+(i1-1)*2] <- varList$pres[c(1:2)+(i1-1)*2] + opt$stepsize[kiter]*(ABres-varList$pres[c(1:2)+(i1-1)*2])
            }
          }
        }
      }
