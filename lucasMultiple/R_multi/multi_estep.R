...
estep.multi<-function(kiter, Uargs, Dargs, opt, mean.phi, varList, DYF, phiM) {
  ...
  U.y<-compute.LLy.multi(phiM,Uargs,Dargs,DYF,varList$pres)
  ...
  for(u in 1:opt$nbiter.mcmc[1]) { # 1er noyau
    ...
    Uc.y<-compute.LLy.multi(phiMc,Uargs,Dargs,DYF,varList$pres)
    deltau<-Uc.y-U.y
  ...
  if(opt$nbiter.mcmc[4]>0 & kiter<opt$nbiter.map) {
    ...
    if(sum(Dargs$modeltype!="structural")==0){
    ...
    for (u in 1:opt$nbiter.mcmc[4]) {
      ...
      Uc.y<-compute.LLy.multi(phiM,Uargs,Dargs,DYF,varList$pres)
