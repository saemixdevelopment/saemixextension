...
compute.LLy.multi<-function(phiM,args,Dargs,DYF,pres) {
  ...
  ##### il faudra gérer ce cas à l'avenir stp  du exp 
  ...
  for(i in 1:length(Dargs$modeltype)) {
    if (Dargs$modeltype[i]=="structural"){
      DYF[args$ind.ioM][ytype==unique_type[i]]<-0.5*((Dargs$yM[ytype==unique_type[i]]-fpred[ytype==unique_type[i]])/gpred[ytype==unique_type[i]])**2+log(gpred[ytype==unique_type[i]])
    } else {
      DYF[args$ind.ioM][ytype==unique_type[i]]<- -fpred[ytype==unique_type[i]]
    }
  }
  U<-colSums(DYF)
  return(U)
...
compute.LLy.multi2<-function(phiM,args,Dargs,DYF,pres) {
  ...
  unique_type = unique(ytype)
  for(ityp in Dargs$etype.exp) fpred[ytype==unique_type[ityp]]<-log(cutoff(fpred[ytype==unique_type[ityp]]))
  ...
  for(itype in 1:length(Dargs$modeltype)) {
    if (Dargs$modeltype[itype]=="structural"){
      DYF[args$ind.ioM][ytype==unique_type[itype]]<-0.5*((Dargs$yM[1:length(Dargs$yobs)][ytype==unique_type[itype]]-fpred[ytype==unique_type[itype]])/gpred[ytype==unique_type[itype]])**2+log(gpred[ytype==unique_type[itype]])
    } else {
      DYF[args$ind.ioM][ytype==unique_type[itype]]<- -fpred[ytype==unique_type[itype]]
    }
  }
  U<-colSums(DYF)
  return(U)
...
## not used : to compute the standard error of non-transformed parameters (if log normal, or logit normal)
...
error<-function(f,ab,etype) { # etype: error model
  g<-f
  unique_type = unique(etype)
  for(i in 1:length(unique_type)) {
    g[etype==unique_type[i]]<-error.typ(f[etype==unique_type[i]],ab[((i-1)*2+1):(i*2)])
  }
  return(g)
}
error.typ<-function(f,ab) {
  g<-cutoff(abs(ab[1]+ab[2]*f)) # Lucas c'est l'original de Saemix multi , je prends l'autre car celui dans SaemixPBPK original 
...
ssq<-function(ab,y,f,etype,all_param=varList$pres,i1=i1) { # Sum of squares; need to put ab first as these parameters are optimised by optim
  all_param[c(1:2)+(i1-1)*2] = ab
  g<-(error(f,ab,etype))
