...
#' 
#' Changes: 
#' - some slots have been removed from the class (the slots related to reading a dataset on disk (automatic, na, sep, header) and covariate transformation (trans.cov))
#' - the new slot outcome now contains the responses associated with each ytype as a list of SaemixOutcome objects
#' - the new slot covariates now contains the covariates in the dataset as a list of SaemixCovariate objects
#' - units for response(s) and covariates are now associated to the corresponding SaemixOutcome and SaemixCovariate objects, so that units is now a vector containing the units for the predictors
#' 
...
#' An object of the SaemixData class can be created by using the function \code{\link{saemixData}} and contains the following slots:
...
#' @slot units a vector of units for the predictors (units for response(s) and covariates are now associated in the corresponding objects outcome and covariates)
...
#' and response. Warning messages will be printed during the object creation if the verbose argument is TRUE
#' and should be read for details.
#' 
#' Important changes from saemix 3.0:
#' - Outcome are now associated with a type (continuous, discrete or event) and stored in the object using the new class SaemixOutcome; the outcomes present in the dataset will be matched to the outcomes required by the model for consistency. Outcome can be created on the fly using the \code{\link{saemixOutcome()}} function.
#' - Multiple responses can be specified through the ytype argument (1 for the first response, 2 for the second, etc...)
#' and each can be given a specific name and a type (continuous, categorical or event)
#' - Covariates are now associated with a type (continuous, binary or categorical) and stored in the object using the new class SaemixCovariate; covariates can be given a unit. Covariates can be created on the fly using the \code{\link{saemixCov()}} function.
#' - the covariates given in name.genetic.covariates will be added to the other covariates as type categorical (or binary if they contain only 2 possible values)
#' 
#' Legacy behaviour:
#' To ensure that previous code still work without too many changes, outcomes and covariates can still be specified 
#' using the same options as previously, but we would like to encourage the users to move on to the more rigorous 
#' way of defining outcomes and covariates (see examples below) as these options will be deprecated in future versions
#' of saemix.
#' 
#' Precedence is as follows in the current version:
#' - if (i) the argument outcome (containing a list of outcomes in the data) is not given, (ii) name.ytype is given 
#' (corresponding to the outcome number) and (iii) the column name.ytype in the data contains 2 or more different values
#' then the function will generate several continuous outcomes y1, y2, ...
#' - if the argument outcome is given and at least one element of the list has units, 
#' the "y" element of the units argument will be ignored
#' - if covariates is empty, name.covariates will be used to generate continuous covariates
#' - if the argument covariates is given, name.covariates will be ignored
#' - if a list of covariates is specified through the covariates argument, name.covariates will be ignored, as well as the 
#' covariate element of the units argument
#' 
...
#' @param covariates a list of objects of type SaemixCovariate containing the covariates in the model. If given, name.covariates will be ignored.
#' @param name.covariates for legacy purposes, a vector containing the names of the covariates present in the dataset; this argument will be ignored if the argument covariates is given.
#' @param name.genetic.covariates name (or number) of the column(s) containing the covariates, if present (otherwise missing). Genetic covariates will automatically be assumed to be categorical (the number of categories will be derived for each covariate from the data).
#' @param name.mdv name of the column containing the indicator for missing variable
#' @param name.cens name of the column containing the indicator for censoring
#' @param name.occ name of the column containing the occasion
#' @param name.ytype name of the column containing the index of the response
...
saemixData<-function(name.data,header,sep,na,name.group,name.predictors, name.response,  name.X, outcome=character(), covariates=c(), name.covariates=c(), name.genetic.covariates=c(), name.mdv="", name.cens="",name.occ="",name.ytype="", units=list(x="",y="",covariates=c()), verbose=TRUE, automatic=TRUE) {
# setting proper types for the SaemixData class
  ...
  if(length(covariates)==0) {
    ...
    for(i in 1:length(covariates)) {
      if(!is(covariates[[i]], "SaemixCovariate")) {
        if(verbose) message("When specified using the covariates argument, please give a list of valid SaemixCovariate elements (see examples)")
        return(NULL)
      }
    }
  }
  x<-new(Class="SaemixData",name.data=name.data,header=header,sep=sep,na=na, name.group=name.group,name.predictors=name.predictors,name.X=name.X, name.response=name.response, covariates=covariates, units=units, name.mdv=name.mdv, name.cens=name.cens, name.occ=name.occ, name.ytype=name.ytype, verbose=verbose, automatic=automatic)
#  showall(x)
    ...
    # Genetic covariates
  	...
  	if(identical(outcome,character())) { # Outcome not given
  	  if(length(unique(x1@data$ytype[x1@data$ytype>0]))==1) { # Only one response
  	    if(length(unique(x1@data[,name.response]))>2) {
  	      x1@outcome<-c(y1="continuous")
  	      names(x1@outcome)<-name.response
  	    ...
  	    for(iresp in 1:nresp)
  	      if(length(unique(x1@data[x1@data$ytype==iresp,name.response]))>2) vec<-c(vec,"continuous") else vec<-c(vec,"categorical")
  	  ...
  	  if(is.null(names(outcome))) { # if only type is given, set outcome names to y1, y2,...
  	    if(length(outcome)==1) names(vec)<-name.response else names(outcome)<-paste("y",1:length(outcome),sep="")
  	  }
