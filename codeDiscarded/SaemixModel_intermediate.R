...
#'     \item{\code{error.model}:}{Object of class \code{"character"}: name of the error model. Valid choices are "constant" (default), "proportional", "combined1", "combined2", "power" and "user" (see equations in User Guide; if "user")}
...
#'     \item{\code{iiv.model}:}{Object of class \code{"matrix"}: a matrix f 0's and 1's giving the structure of the variance-covariance matrix. Defaults to the Identity matrix (diagonal IIV, no correlations between parameters)}
#'     \item{\code{iiv.init}:}{Object of class \code{"matrix"}: a matrix giving the initial estimate for the variance-covariance matrix}
...
# To add (Alexandra)
## iiv.fixed: fixed variance parameters (a matrix or a vector TBD)
...
### Semantics
## fixed (mu,beta)/random (eta => iiv, iov)
## parameters (=psi)/mu (=mu, beta)

# name change
## covariance.model: changed to iiv.model [DONE]
## omega.init: changed to iiv.init [DONE]
## fixed.estim changed to mu.fixed =1-mu.estimated=1-fixed.estim
## name.fixed changed to name.mu [DONE]
## name.random changed to name.iov [for the moment, not done because can include iov and IIV]
## transform.par: change to parameter.distribution ?
...
## error.init: given in outcome as start
...
#' @param iiv.model a square matrix of size equal to the number of parameters in the model, 
#' giving the variance-covariance matrix of the model: 1s correspond to estimated variances (in the diagonal) 
#' or covariances (off-diagonal elements). Defaults to the identity matrix [was covariance.model in saemix 3.0]
#' @param iiv.init a square matrix of size equal to the number of parameters
...
setMethod(
  ...
  definition=function(.Object,model,description,modeltype,psi0, name.response, name.sigma, transform.par,fixed.estim, error.model,covariate.model,iiv.model,iiv.init,error.init, name.modpar, verbose=TRUE){
...
# Checking sizes
    ...
    if(missing(iiv.model) || length(iiv.model)==0) {
      iiv.model<-diag(nrow=npar,ncol=npar)
    } else {
      if(dim(iiv.model)[1]!=dim(iiv.model)[2]) {
        if(verbose) message("Error initialising SaemixModel object:\n   The covariance model needs to be a square matrix, please check dimensions.\n")
      return(.Object)
      }
    }
    nomg<-dim(iiv.model)[1]
    if(nomg!=npar) {
      if(verbose) message("Error initialising SaemixModel object:\n   The covariance model needs to have the same size as the number of parameters.\n")
      return(.Object)
    }
    if(!validate.iiv.model(iiv.model, verbose)) return(.Object)
    if(is.null(colnames(iiv.model))) colnames(iiv.model)<-rownames(iiv.model)<-colnames(psi0)
    .Object@iiv.model<-iiv.model
    index.iiv<-which(diag(iiv.model)>0)
    .Object@index.iiv<-index.iiv
    if(!missing(iiv.init) && length(iiv.init)>0) {
      if(dim(iiv.init)[1]!=dim(iiv.init)[2]) {
        if(verbose) message("Warning:   the matrix giving the initial conditions for the covariance model (iiv.init) needs to be a square matrix. Changing it to the diagonal matrix\n")
        iiv.init<-NULL
      }
    }
    if(missing(iiv.init) || length(iiv.init)==0) {
      iiv.init<-diag(fixed.estim)
...
#      for(i in j1) d[i]<-max(psi0[i]^2,1)
      ...
      iiv.init<-diag(diag.omegi,nrow=npar)
    }
    if(is.null(colnames(iiv.init))) {
      if(dim(iiv.init)[1]==length(colnames(psi0))) colnames(iiv.init)<-rownames(iiv.init)<-colnames(psi0) else message("The dimensions of iiv.init don't agree with the number of parameters")
    }
    .Object@iiv.init<-iiv.init
		if(sum(.Object@fixed.estim*mydiag(.Object@iiv.model))==0) {
			message("Error initialising SaemixModel object:\n")
#			if(sum(mydiag(.Object@iiv.model))==0) cat("At least one parameter with IIV must be included in the model.\n") else cat("At least one parameter with IIV must be estimated and not fixed in the model.\n")
			return(.Object)
		}
...
#   exponential         y = f*exp(a*e)    ( <=>  log(y) = log(f) + a*e )
    ...
    for(i in 1:length(.Object@error.model)) {
      ...
      index.res<-c(index.res,index.res1+2*(i-1))
    }
    .Object@error.init[-index.res]<-0
    .Object@index.res<-index.res
...
setMethod(
  ...
  switch (EXPR=i,
    ...
    "index.iiv"={return(x@index.iiv)},
    "index.res"={return(x@index.res)},
...
setReplaceMethod(
  ...
  switch (EXPR=i,
    ...
    "index.iiv"={x@index.iiv<-value},
    "index.res"={x@index.res<-value},
...
#    try(colnames(tab)<-rownames(tab)<-x@name.modpar)
    ...
    cat("  Error model:",x@error.model,", initial values:",st1[x@index.res],"\n")
    }
...
setMethod("showall","SaemixModel",
  function(object) {
    ...
    if(length(object@name.mu)>0) cat("      fixed parameters: ",object@name.mu,"\n")
    ...
    print(tab,quote=FALSE)
    cat("  Variance-covariance matrix:\n")
    tab<-object@iiv.model
    print(tab,quote=FALSE)
    cat("  Initial estimate for variance-covariance matrix:\n")
    print(object@iiv.init)
    st1<-paste(object@name.sigma,object@error.init,sep="=")
    cat("  Error model:",object@error.model,", initial values:",st1[object@index.res],"\n")
   if(dim(object@covariate.model)[1]>0) {
      cat("  Covariate model:")
    ...
    cat("    Initial values\n")
    print(object@psi0)
    if(length(object@index.mu)>0) cat("      index for fixed parameters: ", object@index.mu,"\n")
    if(length(object@index.beta)>0) cat("      index for covariate parameters: ", object@index.beta,"\n")
    if(length(object@index.iiv)>0) cat("      index for random parameters: ", object@index.iiv,"\n")
    if(length(object@index.res)>0) cat("      index for parameters of residual variability: ", object@index.res,"\n")
...
setMethod("summary","SaemixModel",
  function(object, print=TRUE, ...) {
    ...
    res<-list(model=list(modeltype=object@modeltype,model.function=object@model, error.model=object@error.model),parameters=list(fixed=tab.par, residual.error=tab.res),iiv.model=object@iiv.model, covariate.model=object@covariate.model)
    invisible(res)
 }
)
...
#'   iiv.model=matrix(c(1,0,0,0,1,0,0,0,1),ncol=3,byrow=TRUE),
#'   iiv.init=matrix(c(1,0,0,0,1,0,0,0,1),ncol=3,byrow=TRUE),error.model="constant")
#' 
...
#' class.
#' 
#' Major changes from previous versions: to accomodate multiple outcomes and allow
#' more flexibility for the residual error models, the model definition now explicitely 
#' defines each outcome type.
#' As a result, several arguments have been deprecated
#' including modelType, error.model and error.init. Error models are now defined 
#' along with the nature of outcome (see \code{\link{saemixOutcome}}).
#' 
#' @param model name of the function used to compute the structural model. The
#' function should return a vector of predicted values given a matrix of
#' individual parameters, a vector of indices specifying which records belong
...
#' @param description a character string, giving a brief description of the
#' model or the analysis
#' @param outcome a list of outcomes defined as either continuousOutcome or discreteOutcome 
#' (see \code{\link{saemixOutcome}}). If empty, the program will assume that there is only 
#' one continuous outcome in the model, with a constant error model.
#' Continuous outcomes are defined as a list created by \code{\link{continuousOutcome}}, along with
#' the associated residual error model and the starting values for the residual error parameters. Defaults to constant.
#' Predefined types are constant, proportional, combined1, combined2, exponential, power. 
#' Error models can also be user-defined by associating an error function, the number of parameters and starting values.
#' Discrete outcome are defined through the function \code{\link{discreteOutcome}} and
#' can be binary, event or categorical. For event-type outcome, the argument maxEvents 
#' can be set to define the maximum number of events (defaults to 1 for single event). 
#' For binary or categorical outcomes, the argument levels can be set to define the categories (defaults to c(0,1)).
...
#' @param covariate.model a matrix giving the covariate model. Defaults to no
#' covariate in the model
#' @param iiv.model a square matrix of size equal to the number of
...
saemixModel<-function(model, psi0, description="", outcome=list(), name.response="", name.sigma=character(), transform.par=numeric(),fixed.estim=numeric(),covariate.model=matrix(nrow=0,ncol=0), iiv.model=matrix(nrow=0,ncol=0),iiv.init=matrix(nrow=0,ncol=0), name.modpar=character(), verbose=TRUE) {
