...
\item a combined error model assumes that $g=\sqrt{a^2+b^2\,f^2}$ and $\xi=(a,b)$ (note: this was changed in\saemix 3.0 from the previous model which was $g=a+b\,f$)
...
\noindent {\bf Remark:} It is assumed in \saemix~3.0 that the categorical covariate has only 2 categories (binary covariate). It is also assumed that the variance remains the same for both groups. Covariates with more than 2 categories can always be recoded into (N-1) binary covariates using dummy variables but this is for the moment up to the user.
%It is assumed in \saemix that the correlation matrix of the random effect is the same for all the groups. In other words, only the variances of the random effects can  differ from one group to another.
