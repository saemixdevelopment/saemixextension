...
if(FALSE) {
  namfich<-file.path(datDir,"simulatedBinary_b.txt")
  write.table(datsim,namfich,row.names=F,quote=F)
  namfich<-file.path(datDir,"simulatedBinary_par_b.txt")
  write.table(partab,namfich,row.names=T,quote=F)
}

# Running saemix
saemix.data<-saemixData(name.data=file.path(datDir,"simulatedBinary_b.txt"),
...
# Compare predicted and simulated individual parameters
  parsim<-read.table(file.path(datDir,"simulatedBinary_par_b.txt"),header=T)
  head(binary.fit@results@map.psi)
  head(parsim)
  par(mfrow=c(1,2))
  for(i in 1:2) {
    vec<-parsim[,i]
    if(i==1) vec<-vec+parsim[,1]*param[1]
