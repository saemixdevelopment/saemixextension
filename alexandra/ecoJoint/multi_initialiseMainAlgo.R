...
initialiseMainAlgo.multi<-function(saemix.data,saemix.model,saemix.options) {
...
#             maxim.maxiter=saemix.options$maxim.maxiter,flag.fmin=flag.fmin)
	...
	if(length(grep("structural",saemix.model["modeltype"]))>0) pres<-saemix.model["error.init"] else pres<-c()
...
# 29/05/2020 - changing definition of covariate.estim to 
	...
	if(length(grep("structural",saemix.model["modeltype"]))>0) {
	  theta0<-c(theta0,pres[saemix.model["indx.res"]])
	  allpar0<-c(allpar0,pres[ind.res])        
	}
	...
	varList<-list()
	if(length(grep("structural",saemix.model["modeltype"]))>0) varList$pres<-pres
