...
estep.alex<-function(kiter, Uargs, Dargs, opt, mean.phi, varList, DYF, phiM) {
  ...
  U.yout<-compute.LLy.outcome(phiM,Uargs,Dargs,DYF,varList$pres, kiter=kiter)
  ...
  for(u in 1:opt$nbiter.mcmc[1]) { # 1er noyau
    ...
    Uc.yout<-compute.LLy.outcome(phiMc,Uargs,Dargs,DYF,varList$pres, kiter=kiter)
    #    Uc.eta<-0.5*rowSums(etaMc*(etaMc%*%somega))
    xcal<-(-1)*log(runif(Dargs$NM))
    deltau1<-Uc.yout[[1]]-U.yout[[1]]
    ind1<-which(deltau1<xcal)
    deltau2<-Uc.yout[[2]]-U.yout[[2]]
    ind2<-which(deltau2<xcal)
    ind<-intersect(ind1, ind2)
    indchange1 <- c(indchange1, ind)
    l1<-c(kiter, 1, u, rep(length(ind)/Dargs$NM, nb.etas)) # accepted for both
    keepAR <- rbind(keepAR, l1)
    l1<-c(kiter, 1, u, rep(length(ind1)/Dargs$NM, nb.etas)) # accepted for PK
    keepAR <- rbind(keepAR, l1)
    l1<-c(kiter, 1, u, rep(length(ind2)/Dargs$NM, nb.etas)) # accepted for PD
    keepAR <- rbind(keepAR, l1)
    #    ind1<-c(ind1,ind)
    etaM[ind,]<-etaMc[ind,]
    #    U.eta[ind]<-Uc.eta[ind]
    U.yout[[1]][ind]<-Uc.yout[[1]][ind]
    U.yout[[2]][ind]<-Uc.yout[[2]][ind]
  }
  U.eta<-0.5*rowSums(etaM*(etaM%*%somega))
  if((kiter%%5)==1) print(apply(etaM,2, var))
  
  # Second stage
  # Start by computing U.y, now global over all responsess
  ...
  if(opt$nbiter.mcmc[2]>0) {
    ...
    for (u in 1:opt$nbiter.mcmc[2]) {
      for(vk2 in 1:nb.etas) {
        ...
        l1<-c(kiter, 2, u, nbc2/nt2)
        keepAR <- rbind(keepAR, l1)
      }
    }
    varList$domega2[,nrs2]<-varList$domega2[,nrs2]*(1+opt$stepsize.rw* (nbc2/nt2-opt$proba.mcmc))
    l1<-c(kiter, 2, u+1, nbc2/nt2) # global acceptation ratio (u=3)
    keepnbc<-nbc2
    keepnt<-nt2
    keepAR <- rbind(keepAR, l1)
    if((kiter%%5)==1) cat("domega x",(1+opt$stepsize.rw* (nbc2/nt2-opt$proba.mcmc)),"\n")
    if((kiter%%5)==1) print(apply(etaM,2, var))
  }
  ...
  if(opt$nbiter.mcmc[3]>0) {
    ...
    for (u in 1:opt$nbiter.mcmc[3]) {
      ...
      for(k2 in 1:nb.iter2) {
        ...
        nbc2[vk2]<-nbc2[vk2]+length(ind)
        nt2[vk2]<-nt2[vk2]+Dargs$NM
        l1<-c(kiter, 3, u, nbc2/nt2)
        keepAR <- rbind(keepAR, l1)
      }
    }
    l1<-c(kiter, 3, u+1, nbc2/nt2) # global acceptation ratio (u=3)
    keepAR <- rbind(keepAR, l1)
    l1<-c(kiter, 23, 1, (nbc2+keepnbc)/(nt2+keepnt)) # global acceptation ratio over kernels 2 and 3 (kernel=23)
    keepAR <- rbind(keepAR, l1)
    varList$domega2[,nrs2]<-varList$domega2[,nrs2]*(1+opt$stepsize.rw* (nbc2/nt2-opt$proba.mcmc))
    if(kiter<50 || (kiter%%5)==1) cat("domega2: ",varList$domega2[,nrs2],"\n")
    if((kiter%%5)==1) print(apply(etaM,2, var))
  }
  ...
  return(list(varList=varList,DYF=DYF,phiM=phiM, etaM=etaM, keepAR=keepAR))
