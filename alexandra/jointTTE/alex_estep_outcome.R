################## E-step with 2 outcomes separated ##################

estep.alex.outcome<-function(kiter, Uargs, Dargs, opt, mean.phi, varList, DYF, phiM) {
  ...
  # Caution! Now a list with 2 values 
  U.y<-compute.LLy.outcome(phiM,Uargs,Dargs,DYF,varList$pres, kiter=kiter)
  ...
  for(u in 1:opt$nbiter.mcmc[1]) { # 1er noyau
    ...
    Uc.y<-compute.LLy.outcome(phiMc,Uargs,Dargs,DYF,varList$pres, kiter=kiter)
    xcal<-(-1)*log(runif(Dargs$NM))
    deltau1<-Uc.y[[1]]-U.y[[1]]
    ind1<-which(deltau1<xcal)
    deltau2<-Uc.y[[2]]-U.y[[2]]
    ind2<-which(deltau2<xcal)
    ind<-intersect(ind1, ind2)
    indchange<-c(indchange,ind)
    etaM[ind,]<-etaMc[ind,]
    U.y[[1]][ind]<-Uc.y[[1]][ind]
    U.y[[2]][ind]<-Uc.y[[2]][ind]
  }
  U.eta<-0.5*rowSums(etaM*(etaM%*%somega))
  if((kiter%%5)==1) print(apply(etaM, 2, sd))
  ...
  if(opt$nbiter.mcmc[2]>0) {
    ...
    for (u in 1:opt$nbiter.mcmc[2]) {
      for(vk2 in 1:nb.etas) {
        ...
        phiMc[,varList$ind.eta]<-mean.phiM[,varList$ind.eta]+etaMc
        Uc.y<-compute.LLy.outcome(phiMc,Uargs,Dargs,DYF,varList$pres, kiter=kiter)
        U1.eta<-0.5*rowSums(etaM[,vk2,drop=FALSE]*(etaM[,vk2,drop=FALSE]%*%somega[vk2,vk2]))
        Uc.eta<-0.5*rowSums(etaMc[,vk2,drop=FALSE]*(etaMc[,vk2,drop=FALSE]%*%somega[vk2,vk2]))
        xcal<-(-1)*log(runif(Dargs$NM))
        deltau<-Uc.y[[1]]-U.y[[1]]+Uc.eta-U1.eta
        ind1<-which(deltau<xcal)
        deltau<-Uc.y[[2]]-U.y[[2]]+Uc.eta-U1.eta
        ind2<-which(deltau<xcal)
        ind<-intersect(ind1, ind2)
        indchange<-c(indchange,ind)
  ...
  if(opt$nbiter.mcmc[3]>0) {
    ...
    for (u in 1:opt$nbiter.mcmc[3]) {
      ...
      for(k2 in 1:nb.iter2) {
        ...
        phiMc[,varList$ind.eta]<-mean.phiM[,varList$ind.eta]+etaMc
        Uc.y<-compute.LLy.outcome(phiMc,Uargs,Dargs,DYF,varList$pres, kiter=kiter)
#        U.eta<-0.5*rowSums(etaM[,vk2,drop=FALSE]*(etaM[,vk2,drop=FALSE]%*%somega[vk2,vk2]))
#        Uc.eta<-0.5*rowSums(etaMc[,vk2,drop=FALSE]*(etaMc[,vk2,drop=FALSE]%*%somega[vk2,vk2]))
        Uc.eta<-0.5*rowSums(etaMc*(etaMc%*%somega))
        xcal<-(-1)*log(runif(Dargs$NM))
        deltau<-Uc.y[[1]]-U.y[[1]]+Uc.eta-U.eta
        ind1<-which(deltau<xcal)
        deltau<-Uc.y[[2]]-U.y[[2]]+Uc.eta-U.eta
        ind2<-which(deltau<xcal)
        ind<-intersect(ind1, ind2)
        indchange<-c(indchange,ind)
        #        ind1<-c(ind1,ind)
  ...
  if(opt$nbiter.mcmc[4]>0 & kiter<opt$nbiter.map) {
    ...
    for (u in 1:opt$nbiter.mcmc[4]) {
      ...
      Uc.y<-compute.LLy.outcome(phiM,Uargs,Dargs,DYF,varList$pres, kiter=kiter)
  ...
  if((kiter%%5)==1) cat("nmod=",length(unique(indchange))," --")
...
compute.LLy.outcome <- function(phiM, args, Dargs, DYF, pres, kiter) {
  ...
  # Separating both Uy
  DYF.rep1<-DYF[args$ind.ioM[Dargs$XM$ytype==1]]
  id.rep1<-Dargs$IdM[args$ind.ioM[Dargs$XM$ytype==1]]
  DYF.rep2<-DYF[args$ind.ioM[Dargs$XM$ytype==2]]
  id.rep2<-Dargs$IdM[args$ind.ioM[Dargs$XM$ytype==2]]
  Uy.rep1 <- tapply(DYF.rep1, id.rep1, sum)
  Uy.rep2 <- tapply(DYF.rep2, id.rep2, sum)
  
  U <- list(Uy.rep1, Uy.rep2)
  return(U)
}
