...
estep<-function(kiter, Uargs, Dargs, opt, mean.phi, varList, DYF, phiM) {
  ...
  for(u in 1:opt$nbiter.mcmc[1]) { # 1er noyau
    ...
    U.y[ind]<-Uc.y[ind]
    ind1<-c(ind1, ind)
    l1<-c(kiter, 1, u, rep(length(ind)/Dargs$NM, nb.etas))
  ...
  # no ind here as not used normally
  ...
  if((kiter%%5)==1) cat("kiter=",kiter,"nmod=",length(unique(indchange1)),"/",length(unique(indchange2)),"/",length(unique(indchange3))," --", exp(mean.phiM[1,]),varList$pres[Uargs$ind.res],"\n")
