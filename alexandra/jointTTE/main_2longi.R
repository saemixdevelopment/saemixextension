...
saemix.alex<-function(model,data,control=list()) {
  ...
  xinit<-initialiseMainAlgo.alex(saemix.data,saemix.model,saemix.options)
  ...
  suffStat<-list(statphi1=0,statphi2=0,statphi3=0,statrese1=0,statrese2=0)
  ...
  for (kiter in 1:saemix.options$nbiter.tot) { # Iterative portion of algorithm
    #  for (kiter in 1:100) { # Iterative portion of algorithm    #saemix.options$nbiter.tot
#  for (kiter in 1:150) { # Iterative portion of algorithm    #saemix.options$nbiter.tot
#  for (kiter in 1:5) { # Iterative portion of algorithm    #saemix.options$nbiter.tot
    #  for (kiter in 6:20) { # Iterative portion of algorithm    #saemix.options$nbiter.tot
    #  for (kiter in 1:10) { # Iterative portion of algorithm    #saemix.options$nbiter.tot
    #  for (kiter in 11:150) { # Iterative portion of algorithm    #saemix.options$nbiter.tot
...
#    xmcmc<-estep.alex.outcome(kiter, Uargs, Dargs, opt, mean.phi, varList, DYF, phiM)
    xmcmc<-estep.alex(kiter, Uargs, Dargs, opt, mean.phi, varList, DYF, phiM)
    ...
    if(opt$stepsize[kiter]>0) {
      ...
      xstoch<-mstep.alex(kiter, Uargs, Dargs, opt, structural.model, DYF, phiM, varList, phi, betas, suffStat)
    ...
    if((kiter%%5)==1) cat("kiter=",kiter,theta,"\n") # print every 5 iterations
    # End of loop on kiter
  }
#  return(invisible(list(allpar=allpar, varList=varList, Uargs=Uargs, Dargs=Dargs, DYF=DYF, phiM=phiM, mean.phi=mean.phi, opt=opt, phi=phi, betas=betas, suffStat=suffStat)))
