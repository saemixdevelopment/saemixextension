...
# data
data_joint <- read.csv("C:/Users/AlexandraLAVALLEY/Documents/GitHub/saemixextension/alexandra/joint_alex/datas/comp_facile2.csv", header=TRUE)
...
# joint TTE  
param<-c(15,0.1,0.2,0.02) # b0, b1, p, r
jointTTE<-saemixModel(model=JMmodel,description="JM with CR",modeltype=c("structural","likelihood"),
                      psi0=matrix(param,ncol=4,byrow=TRUE,dimnames=list(NULL, c( "b0", "b1", "p", "r"))),
                      transform.par=c(0,0,1,1), covariance.model=diag(c(1,1,0,0)),
                      fixed.estim = c(1,1,1,1), omega.init = diag(c(1,1,1,1)))
