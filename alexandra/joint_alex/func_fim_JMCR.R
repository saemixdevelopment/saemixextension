...
fim.saemix<-function(saemixObject) {
  ...
  F2<-array(data=0,dim=c(length(saemix.data@data[["ytype"]][saemix.data@data[["ytype"]]==1]),2,length(coefphi))) # 2 par longi !! a adapter !!!! 
  ...
  for (l in 1:length(coefphi)) {
    ...
    F2[,,l] = F[which(ytype==1),1:2,l]  ## a adapter aussi
  }
  ...
  ev1 = saemix.data@data$id[saemix.data@data$ytype==2 & saemix.data@data$obs==1]
  ev2 = saemix.data@data$id[saemix.data@data$ytype==2 & saemix.data@data$obs==2]
  ev0 = saemix.data@data$id[saemix.data@data$ytype==2 & saemix.data@data$obs==0]
  Ti = saemix.data@data$time[saemix.data@data$ytype==2]
  
  p1 = saemix.res@fixed.effects[3]
  g1 = saemix.res@fixed.effects[4]
  alpha1 = saemix.res@fixed.effects[5]
  
  ##################### Pour p1 
  # si ev = 1
  
  b0i = saemix.res@map.psi[,1][ev1]
  b1i = saemix.res@map.psi[,2][ev1]
  Ti1 = Ti[ev1]
  ...
  for (i in 1:length(ev1)){
    ...
    b1 = b1i[i]
    ti = Ti1[i]
    f_2 = function(t) g1*exp(-g1*t)*exp(alpha1*(b0+b1*t))*2*(1-exp(-g1*t))/(1-p1*(1-exp(-g1*t)))**3
  ...
  b0i0 = saemix.res@map.psi[,1][ev0]
  b1i0 = saemix.res@map.psi[,2][ev0]
  Ti0 = Ti[ev0]
  ...
  for (i in 1:length(ev0)){
    ...
    b1 = b1i[i]
    ti = Ti0[i]
    f = function(t) p1*g1*exp(alpha1*b0)/(1-p1)*(exp((alpha1*b1-g1)*t)/(alpha1*b1-g1)*hyperg_2F1(1,1-alpha1*b1/g1,2-alpha1*b1/g1,-p1*exp(-g1*t)/(1-p1))-1/(alpha1*b1-g1)*hyperg_2F1(1,1-alpha1*b1/g1,2-alpha1*b1/g1,-p1/(1-p1)))
    f_1 = function(t) g1*exp(-g1*t)*exp(alpha1*(b0+b1*t))/((1-p1*(1-exp(-g1*t)))**2)
    f_2 = function(t) g1*exp(-g1*t)*exp(alpha1*(b0+b1*t))*2*(1-exp(-g1*t))/((1-p1*(1-exp(-g1*t)))**3)
    ...
    F1 =  1-exp(-p1*g1*exp(alpha1*b0)/(1-p1)*(exp((alpha1*b1-g1)*1000)/(alpha1*b1-g1)*hyperg_2F1(1,1-alpha1*b1/g1,2-alpha1*b1/g1,-p1*exp(-g1*1000)/(1-p1))-1/(alpha1*b1-g1)*hyperg_2F1(1,1-alpha1*b1/g1,2-alpha1*b1/g1,-p1/(1-p1))))
  ...
  for (i in 1:length(ev1)){
    ...
    b1 = b1i[i]
    ti = Ti1[i]
    f_2 = function(t) ((p1-1)*t*exp(g1*t)*((p1-1)*exp(g1*t)*(g1*t-2)+p1*(g1*t+2))*p1*exp(g1*t+alpha1*(b0+b1*t)))/((p1-1)*exp(g1*t)-p1)**3
    part2 = c(part2,integrate(f_2,0,ti)$value)
  }
  ...
  for (i in 1:length(ev0)){
    ...
    ti = Ti0[i]
    f = function(t) p1*g1*exp(alpha1*b0)/(1-p1)*(exp((alpha1*b1-g1)*t)/(alpha1*b1-g1)*hyperg_2F1(1,1-alpha1*b1/g1,2-alpha1*b1/g1,-p1*exp(-g1*t)/(1-p1))-1/(alpha1*b1-g1)*hyperg_2F1(1,1-alpha1*b1/g1,2-alpha1*b1/g1,-p1/(1-p1)))
    f_1 = function(t) p1*exp(alpha1*(b0+b1*t))*((p1-1)*exp(g1*t)*(g1*t-1)+p1)/((p1-(p1-1)*exp(g1*t))**2)
    f_2 = function(t) ((p1-1)*t*exp(g1*t)*((p1-1)*exp(g1*t)*(g1*t-2)+p1*(g1*t+2))*p1*exp(g1*t+alpha1*(b0+b1*t)))/((p1-1)*exp(g1*t)-p1)**3
  ...
  for (i in 1:length(ev1)){
    ...
    b1 = b1i[i]
    ti = Ti1[i]
    f_2 = function(t) (g1*p1*exp(-g1*t)*(b0+b1*t)**2 *exp(alpha1*(b0+b1*t)))/(1-p1*(1-exp(-g1*t)))
    part2 = c(part2,integrate(f_2,0,ti)$value)
  }
  ...
  for (i in 1:length(ev0)){
    ...
    ti = Ti0[i]
    f = function(t) p1*g1*exp(alpha1*b0)/(1-p1)*(exp((alpha1*b1-g1)*t)/(alpha1*b1-g1)*hyperg_2F1(1,1-alpha1*b1/g1,2-alpha1*b1/g1,-p1*exp(-g1*t)/(1-p1))-1/(alpha1*b1-g1)*hyperg_2F1(1,1-alpha1*b1/g1,2-alpha1*b1/g1,-p1/(1-p1)))
    f_1 = function(t) (g1*p1*exp(-g1*t)*(b0+b1*t)*exp(alpha1*(b0+b1*t)))/(1-p1*(1-exp(-g1*t)))
    f_2 = function(t) (g1*p1*exp(-g1*t)*(b0+b1*t)**2 *exp(alpha1*(b0+b1*t)))/(1-p1*(1-exp(-g1*t)))
  ...
  part1 = Ti1*exp(g1*Ti1)/(p1-(p1-1)*exp(g1*Ti1))
  ...
  for (i in 1:length(ev1)){
    ...
    b1 = b1i[i]
    ti = Ti1[i]
    f_2 = function(t) ((g1*p1*t+(p1-1)*exp(g1*t)*(g1*t-1)+p1)*exp(g1*t+alpha1*(b0+b1*t)))/((p1-1)*exp(g1*t)-p1)**3
    part2 = c(part2,integrate(f_2,0,ti)$value)
  }
  ...
  for (i in 1:length(ev0)){
    ...
    ti = Ti0[i]
    f = function(t) p1*g1*exp(alpha1*b0)/(1-p1)*(exp((alpha1*b1-g1)*t)/(alpha1*b1-g1)*hyperg_2F1(1,1-alpha1*b1/g1,2-alpha1*b1/g1,-p1*exp(-g1*t)/(1-p1))-1/(alpha1*b1-g1)*hyperg_2F1(1,1-alpha1*b1/g1,2-alpha1*b1/g1,-p1/(1-p1)))
    f_1p = function(t) g1*exp(-g1*t)*exp(alpha1*(b0+b1*t))/((1-p1*(1-exp(-g1*t)))**2)
    f_1g = function(t) p1*exp(alpha1*(b0+b1*t))*((p1-1)*exp(g1*t)*(g1*t-1)+p1)/((p1-(p1-1)*exp(g1*t))**2)
    f_2 = function(t) ((g1*p1*t+(p1-1)*exp(g1*t)*(g1*t-1)+p1)*exp(g1*t+alpha1*(b0+b1*t)))/((p1-1)*exp(g1*t)-p1)**3
  ...
  for (i in 1:length(ev1)){
    ...
    b1 = b1i[i]
    ti = Ti1[i]
    f_2 = function(t) ((g1*(b0+b1*t)*exp(g1*t+alpha1*(b0+b1*t))))/(p1-(p1-1)*exp(g1*t))**2
    part2 = c(part2,integrate(f_2,0,ti)$value)
  }
  ...
  for (i in 1:length(ev0)){
    ...
    f = function(t) p1*g1*exp(alpha1*b0)/(1-p1)*(exp((alpha1*b1-g1)*t)/(alpha1*b1-g1)*hyperg_2F1(1,1-alpha1*b1/g1,2-alpha1*b1/g1,-p1*exp(-g1*t)/(1-p1))-1/(alpha1*b1-g1)*hyperg_2F1(1,1-alpha1*b1/g1,2-alpha1*b1/g1,-p1/(1-p1)))
    f_1p = function(t) g1*exp(-g1*t)*exp(alpha1*(b0+b1*t))/((1-p1*(1-exp(-g1*t)))**2)
    f_1a = function(t) (g1*p1*exp(-g1*t)*(b0+b1*t)*exp(alpha1*(b0+b1*t)))/(1-p1*(1-exp(-g1*t)))
    f_2 = function(t) ((g1*exp(-g1*t)*(b0+b1*t)*exp(alpha1*(b0+b1*t))))/((1-p1*(1-exp(g1*t)))**2)
  ...
  for (i in 1:length(ev1)){
    ...
    b1 = b1i[i]
    ti = Ti1[i]
    f_2 = function(t) ((p1*(b0+b1*t)*((p1-1)*exp(g1*t)*exp(g1*t-1)+p1)*exp(alpha1*(b0+b1*t))))/(p1-(p1-1)*exp(g1*t))**2
    part2 = c(part2,integrate(f_2,0,ti)$value)
  }
  ...
  for (i in 1:length(ev0)){
    ...
    ti = Ti0[i]
    f = function(t) p1*g1*exp(alpha1*b0)/(1-p1)*(exp((alpha1*b1-g1)*t)/(alpha1*b1-g1)*hyperg_2F1(1,1-alpha1*b1/g1,2-alpha1*b1/g1,-p1*exp(-g1*t)/(1-p1))-1/(alpha1*b1-g1)*hyperg_2F1(1,1-alpha1*b1/g1,2-alpha1*b1/g1,-p1/(1-p1)))
    f_1g = function(t) p1*exp(alpha1*(b0+b1*t))*((p1-1)*exp(g1*t)*(g1*t-1)+p1)/(((p1-1)*exp(g1*t)-p1)**2)
    f_1a = function(t) (g1*p1*exp(-g1*t)*(b0+b1*t)*exp(alpha1*(b0+b1*t)))/(1-p1*(1-exp(-g1*t)))
    f_2 = function(t) ((p1*(b0+b1*t)*((p1-1)*exp(g1*t)*exp(g1*t-1)+p1)*exp(alpha1*(b0+b1*t))))/(p1-(p1-1)*exp(g1*t))**2
  ...
  mat = matrix(c(var_p1,covar_p1g1,covar_p1alpha1,covar_p1g1,var_g1,covar_g1alpha1,covar_p1alpha1,covar_g1alpha1,var_alpha1),byrow = T,nrow = 3)
  matinv = solve(mat)
